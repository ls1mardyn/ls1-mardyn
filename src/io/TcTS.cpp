			...
			for(unsigned j=0; j < fl_units[1][l]; j++) {
				for(unsigned k=0; k < fl_units[2][l]; k++) {
					for(unsigned d=0; d < 3; d++) {
						if (l == 0)
							fill0[((fl_units[0][0] * i + j) * fl_units[1][0] + k) * fl_units[2][0] + d] = true;
						else fill1[((fl_units[0][1] * i + j) * fl_units[1][1] + k) * fl_units[2][1] + d] = true;
					}
				}
			}
		}
				...
				for(unsigned j=0; j < fl_units[1][l]; j++) {
					for(unsigned k=0; k < fl_units[2][l]; k++) {
						for(unsigned d=0; d < 3; d++) {
							if(pswap >= rnd->rnd()) {
								if (((l == 0)
										&& fill0[((fl_units[0][0] * i + j) * fl_units[1][0] + k) * fl_units[2][0] + d])
										|| ((l == 1)
												&& fill1[((fl_units[0][1] * i + j) * fl_units[1][1] + k)
														* fl_units[2][1] + d]))
									N[l]--;
								if (l == 0)
									fill0[((fl_units[0][0] * i + j) * fl_units[1][0] + k) * fl_units[2][0] + d] = tswap;
								else
									fill1[((fl_units[0][1] * i + j) * fl_units[1][1] + k) * fl_units[2][1] + d] = tswap;
								if (tswap)
									N[l]++;
							}
						}
	...
	for(unsigned l=0; l < 2; l++) {
		for(unsigned i=0; i < fl_units[0][l]; i++) {
			for(unsigned j=0; j < fl_units[1][l]; j++) {
				for(unsigned k=0; k < fl_units[2][l]; k++) {
					for(unsigned d=0; d < 3; d++) {
						if (((l == 0) && fill0[((fl_units[0][0] * i + j) * fl_units[1][0] + k) * fl_units[2][0] + d])
								|| ((l == 1)
										&& fill1[((fl_units[0][1] * i + j) * fl_units[1][1] + k) * fl_units[2][1] + d])) {
							double q[3];
							q[0] = i * fl_unit[0][l];
							q[1] = j * fl_unit[1][l];
							q[2] = k * fl_unit[2][l];
