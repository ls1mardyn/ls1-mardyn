...
void MPICheckpointWriter::doOutput( ParticleContainer* particleContainer, DomainDecompBase* domainDecomp, Domain* domain, unsigned long simstep, list<ChemicalPotential>* lmu )
{
	...
	if( simstep % _writeFrequency == 0 ) {
		...
		MPI_File mpifh;
		MPI_CHECK( MPI_File_open(MPI_COMM_WORLD, const_cast<char*>(filename.c_str()), MPI_MODE_WRONLY|MPI_MODE_CREATE, MPI_INFO_NULL, &mpifh) );	// arg 2 type cast due to non-standard MPI 3 implementations (should be const char*)
		//MPI_CHECK( MPI_File_preallocate( mpifh, size) )
		MPI_CHECK( MPI_File_set_view(mpifh, 0, MPI_BYTE, MPI_BYTE, const_cast<char*>(mpidatarep), MPI_INFO_NULL) );	// arg 5 type cast due to non-standard MPI 3 implementations (should be const char*)
		...
		{	// the first part of header will be written by rank 0 only
			...
			MPI_CHECK( MPI_File_write(mpifh, const_cast<void*>(_magicVersion), strlen(_magicVersion)+1, MPI_CHAR, &mpistat) );	// arg 2 type cast due to non-standard MPI 3 implementations (should be const void*)
			...
			MPI_CHECK( MPI_File_write(mpifh, const_cast<void*>(&_endiannesstest), 1, MPI_INT, &mpistat) );	// arg 2 type cast due to non-standard MPI 3 implementations (should be const void*)
			...
			//MPI_CHECK( MPI_File_seek(mpifh,mpioffset,MPI_SEEK_SET) );
			MPI_CHECK( MPI_File_write(mpifh, const_cast<void*>("ICRVQD"), 6+1, MPI_CHAR, &mpistat) );	// arg 2 type cast due to non-standard MPI 3 implementations (should be const void*)
			mpioffset+=7;
			//
			//MPI_CHECK( MPI_File_write_at(mpifh,mpioffset,"BB",3,MPI_CHAR,&mpistat) );
			//MPI_CHECK( MPI_File_seek(mpifh,mpioffset,MPI_SEEK_SET) );
			MPI_CHECK( MPI_File_write(mpifh, const_cast<void*>("BB"), 2+1, MPI_CHAR, &mpistat) );	// arg 2 type cast due to non-standard MPI 3 implementations (should be const void*)
			mpioffset+=3;
		...
		*/
		...
		MPI_CHECK( MPI_File_set_view(mpifh, mpioffset, mpidtParticleM, mpidtParticleM, const_cast<char*>(mpidatarep), MPI_INFO_NULL) );	// arg 5 type cast due to non-standard MPI 3 implementations (should be const char*)
