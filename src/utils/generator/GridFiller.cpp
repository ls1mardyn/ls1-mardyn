...
void GridFiller::readXML(XMLfileUnits& xmlconfig) {
	...
	global_log->info() << "Lattice occupancy: " << _latticeOccupancy << std::endl;
	double rho = 0.0;
	if(xmlconfig.getNodeValueReduced("density", rho)) {
		global_log->info() << "Density: " << rho << std::endl;
		rho /= _latticeOccupancy;
		global_log->info() << "Initializing cubic lattice with density (fully occupied): " << rho << std::endl;
		global_log->warning() << "Initializing cubic lattice with density overwrites previously set lattice system and vectors." << std::endl;
		long num_molecules_per_crystal_cell = _basis.numMolecules() * _lattice.numCenters();
		global_log->debug() << "Number of molecules per crystall cell: " << num_molecules_per_crystal_cell << std::endl;
	...
	global_log->info() << "Origin: " << _origin[0] << ", " << _origin[1] << ", " << _origin[2] << std::endl;
}
