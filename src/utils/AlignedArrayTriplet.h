...
public:
	...
	T* xBegin() { return _numEntriesPerArray > 0 ? this->vec->data() + (0 * _numEntriesPerArray) : nullptr; }
	T* yBegin() { return _numEntriesPerArray > 0 ? this->vec->data() + (1 * _numEntriesPerArray) : nullptr; }
	T* zBegin() { return _numEntriesPerArray > 0 ? this->vec->data() + (2 * _numEntriesPerArray) : nullptr; }
	T* xBegin() const { return _numEntriesPerArray > 0 ? this->vec->data() + (0 * _numEntriesPerArray) : nullptr; }
	T* yBegin() const { return _numEntriesPerArray > 0 ? this->vec->data() + (1 * _numEntriesPerArray) : nullptr; }
	T* zBegin() const { return _numEntriesPerArray > 0 ? this->vec->data() + (2 * _numEntriesPerArray) : nullptr; }

	T& x(size_t i) { mardyn_assert(i < _numEntriesPerArray); return (*this->vec)[i + 0 * _numEntriesPerArray]; }
	T& y(size_t i) { mardyn_assert(i < _numEntriesPerArray); return (*this->vec)[i + 1 * _numEntriesPerArray]; }
	T& z(size_t i) { mardyn_assert(i < _numEntriesPerArray); return (*this->vec)[i + 2 * _numEntriesPerArray]; }
	T& x(size_t i) const { mardyn_assert(i < _numEntriesPerArray); return (*this->vec)[i + 0 * _numEntriesPerArray]; }
	T& y(size_t i) const { mardyn_assert(i < _numEntriesPerArray); return (*this->vec)[i + 1 * _numEntriesPerArray]; }
	T& z(size_t i) const { mardyn_assert(i < _numEntriesPerArray); return (*this->vec)[i + 2 * _numEntriesPerArray]; }
	...
	T& linearCrossAccess(size_t i) { mardyn_assert(i < 3*_numEntriesPerArray); return (*this->vec)[i];}
	...
	size_t resize_zero_shrink(size_t exact_size, bool zero_rest_of_CL = false, bool allow_shrink = false) {
		...
		bool need_resize = size_rounded_up_x3 > this->vec->capacity() or (allow_shrink and size_rounded_up_x3 < this->vec->capacity());
