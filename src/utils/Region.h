...
// class SphereComplementRegion : public Region
// {
// public:
// 	SphereComplementRegion(ControlInstance* parent);
// 	SphereComplementRegion(ControlInstance* parent, double dCtr[3], double radius, double dLC[3], double dUC[3]);
...
// 	std::array<double,3> GetCenter() {return _dCenter;}
// 	double GetRadius() {return _dRadius;}
// 	double GetRadiusSquared() {return _dRadiusSquared; _dRadiusSquared = std::pow(radius, 2);}
...
// 	// bool PositionIsInside(const uint16_t& nDim, const double& dPos) {return (dPos > _dLowerCorner.at(nDim) ) && (dPos < _dUpperCorner.at(nDim) );}
// 	bool PositionIsInside(double* dPos) {
// 		double dPosRelative = dPos-_dCenter;
// 		double distanceFromCenterSquared = std::pow(dPosRelative[0],2) + std::pow(dPosRelative[1], 2) + std::pow(dPosRelative[2],2);

// 		if(distanceFromCenterSquared <= _dRadiusSquared) return false;
// 		else if	( !(dPos[0] > _dLowerCorner.at(0) && dPos[0] < _dUpperCorner.at(0) ) ) return false;
// 		else if ( !(dPos[1] > _dLowerCorner.at(1) && dPos[1] < _dUpperCorner.at(1) ) ) return false;
// 		else if	( !(dPos[2] > _dLowerCorner.at(2) && dPos[2] < _dUpperCorner.at(2) ) ) return false;
// 		else	return true;
// 	}
...
// 		os << "----------------------------------------------------------------" << std::endl;
// 		os << "ID: " << _nID << std::endl;
// 		os << "regionType: ResidualRegion (Box - Sphere)";
// 		std::array<double,3> lc = this->GetLowerCorner();
// 		std::array<double,3> uc = this->GetUpperCorner();
// 		os << "lowerCorner: " << lc[0] << " " << lc[1] << " " << lc[2] << std::endl;
// 		os << "upperCorner: " << uc[0] << " " << uc[1] << " " << uc[2] << std::endl;
// 		os << "radiusRemoved: " << this->GetRadius(0) << " " << this->GetRadius(1) << " " << this->GetRadius(2) << std::endl;
// 		os << "----------------------------------------------------------------" << std::endl;
// 	}
// 	double GetVolume()
// 	{
// 		double dVolume = this->GetWidth(0) * this->GetWidth(1) * this->GetWidth(2);
// 		dVolume -=  4.18879020479 * std::pow(this->GetRadius,3);
// 		return dVolume;
// 	}
...
// 	SphereComplementRegionObs(ControlInstance* parent, double dLC[3], double dUC[3] );
