...
public:
  ...
  double magnitude2() { return m_qw*m_qw+m_qx*m_qx+m_qy*m_qy+m_qz*m_qz; }
  void scale(double s) { m_qw*=s; m_qx*=s; m_qy*=s; m_qz*=s; }
  void scaleinv(double s) { m_qw/=s; m_qx/=s; m_qy/=s; m_qz/=s; }
  void normalize() { scaleinv(sqrt(magnitude2())); }
  void conjugate() { m_qx=-m_qx; m_qy=-m_qy; m_qz=-m_qz; }
  void inverse() { conjugate(); scaleinv(magnitude2()); }
  void add(const Quaternion& q)
    { m_qw+=q.m_qw; m_qx+=q.m_qx; m_qy+=q.m_qy; m_qz+=q.m_qz;}

  void operator *=(const Quaternion& q);
  void rotate(const double d[3], double drot[3]) const;
  void rotate(double d[3]) const
    { double drot[3]; rotate(d,drot); d[0]=drot[0];d[1]=drot[1];d[2]=drot[2];}
  void rotateinv(const double d[3], double drot[3]) const;
  void rotateinv(double d[3]) const
    { double drot[3]; rotateinv(d,drot); d[0]=drot[0];d[1]=drot[1];d[2]=drot[2];}
