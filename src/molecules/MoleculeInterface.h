...
public:
	 ...
	 * This method generates data matching MoleculeInterface::deserialize.
	...
	/**
	 * @brief Deserializes byte data and sets this instance's values accordingly.
	 * This method sets a molecule instance's values from a byte array (std::vector<char>).
	 * Returned is an iterator to the first element of the byte array which has *not* been
	 * processed. This method matches data generated by MoleculeInterface::serialize.
	 * Reimplemented in FullMolecule::deserialize and MoleculeRMM::deserialize
	 * @param[in] first Iterator of the first element in the source buffer
	 * @return An iterator pointing to the element of source buffer located after the processed data
	 */
	virtual std::vector<char>::iterator deserialize(std::vector<char>::iterator first) = 0;
	/**
