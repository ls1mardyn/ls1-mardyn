...
bool ParticleContainer::addHaloParticleImpl(Molecule& particle, bool inBoxCheckedAlready, bool checkWhetherDuplicate, const bool& rebuildCaches) {
	mardyn_assert(not particle.inBox(_boundingBoxMin,_boundingBoxMax));
	return addParticleImpl(particle, inBoxCheckedAlready, checkWhetherDuplicate, rebuildCaches);
}
