...
void RDFForceIntegratorExact::getScalingFactor(double* mol_r, double* site_r,
	...
	for (int idx_alpha = 0; idx_alpha < _n_alpha; idx_alpha++) {
		...
		curr_x = x + _extension * sin(alpha * PI / 180);
		other_site_x = x + 2 * _extension * sin(alpha * PI / 180);

		for (int idx_phi = 0; idx_phi < _n_alpha; idx_phi++) {
			phi = idx_phi * _d_alpha;
			curr_y = y + _extension * cos(alpha * PI / 180) * cos(phi * PI
					/ 180);

			curr_z = z + _extension * cos(alpha * PI / 180) * sin(phi * PI
					/ 180);

			other_site_y = y + 2 * _extension * cos(alpha * PI / 180) * cos(phi
					* PI / 180);

			other_site_z = z + 2 * _extension * cos(alpha * PI / 180) * sin(phi
					* PI / 180);
			...
			if (curr_r < _rc) {
				...
				curr_bin = (int) (other_site_r
						* globalNondecliningSiteADist[0][0].size() / (_rc + 2
						* _extension) - 0.5);
				curr_g[0]
						= globalNondecliningSiteADist[0][site_i * 2 + 0][curr_bin];
				curr_g[1]
						= globalNondecliningSiteADist[0][site_i * 2 + 1][curr_bin];
...
void RDFForceIntegratorExact::traverseMolecules() {
	...
	_dx = _dy = _dz = _dr = _dn = currentMolecule->getSigma() / 10;
...
void RDFForceIntegratorExact::integrateRDFSiteCartesian(double xlim[2],
	...
	double currf[3], absmold, allowed_dist, small_y, small_z, abssited, g = 0;
	double scale[2] = { 1, 1 };
	...
	for (double x = xlim[0] + _dx / 2; x + _dx / 2 <= xlim[1]; x += _dx) {
		for (double y = ylim[0] + _dy / 2; y + _dy / 2 <= ylim[1]; y += _dy) {
			for (double z = zlim[0] + _dz / 2; z + _dz / 2 <= zlim[1]; z += _dz) {
				...
				if (plane == 0 && std::abs(x) < _extension) {
					/*
					 idx_level = (int) (molr[0] / _dn + 0.5);
					 idx_normal = (int) ((x + _extension) / _dn + 0.5);
					 idx_r = (int) (std::sqrt((y - molr[1]) * (y - molr[1]) + (z
					 - molr[2]) * (z - molr[2])) / _dn + 0.5);
					 */
				...
				if (mol->numSites() == 1) {
					...
					for (unsigned int j = 0; j < mol->numLJcenters(); j++) {
						...
						g *= scale[j];
						double currPot = 0;
						PotForceLJ(sited, abssited * abssited, 24
								* mol->getEps(), mol->getSigma()
								* mol->getSigma(), currf, currPot);
