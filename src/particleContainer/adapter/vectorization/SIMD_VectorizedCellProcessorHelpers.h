...
static vcp_inline
void unpackEps24Sig2(DoubleVec& eps_24, DoubleVec& sig2, const DoubleArray& eps_sigI,
	...
	const DoubleVec e0s0 = DoubleVec::aligned_load_mask(eps_sigI + 2 * id_j_shifted[0], memoryMask_first_second);
	const DoubleVec e1s1 = DoubleVec::aligned_load_mask(eps_sigI + 2 * id_j_shifted[1], memoryMask_first_second);
	const DoubleVec e2s2 = DoubleVec::aligned_load_mask(eps_sigI + 2 * id_j_shifted[2], memoryMask_first_second);
	const DoubleVec e3s3 = DoubleVec::aligned_load_mask(eps_sigI + 2 * id_j_shifted[3], memoryMask_first_second);

	const DoubleVec e0e1 = DoubleVec::unpack_lo(e0s0, e1s1);
	const DoubleVec s0s1 = DoubleVec::unpack_hi(e0s0, e1s1);
	const DoubleVec e2e3 = DoubleVec::unpack_lo(e2s2, e3s3);
	const DoubleVec s2s3 = DoubleVec::unpack_hi(e2s2, e3s3);
...
template <class MaskGatherChooser>
static vcp_inline
void unpackShift6(DoubleVec& shift6, const DoubleArray& shift6I,
	...
	const DoubleVec sh0 = DoubleVec::aligned_load_mask(shift6I + id_j_shifted[0], memoryMask_first);
	const DoubleVec sh1 = DoubleVec::aligned_load_mask(shift6I + id_j_shifted[1], memoryMask_first);
 ...
 */
static vcp_inline
void hSum_Add_Store( double * const mem_addr, const DoubleVec & a ) {
	...
	static const __m256i memoryMask_first = _mm256_set_epi32(0, 0, 0, 0, 0, 0, 1<<31, 0);
	const DoubleVec a_t1 = _mm256_permute2f128_pd(a, a, 0x1);
	const DoubleVec a_t2 = _mm256_hadd_pd(a, a_t1);
	const DoubleVec a_t3 = _mm256_hadd_pd(a_t2, a_t2);
	_mm256_maskstore_pd(
		mem_addr,
		memoryMask_first,
			a_t3 + DoubleVec::aligned_load_mask(mem_addr, memoryMask_first)
	);
...
void load_hSum_Store_Clear(double * const compact, double * const wide) {
	DoubleVec wide_reg = DoubleVec::aligned_load(wide);
	hSum_Add_Store(compact, wide_reg);
	wide_reg = DoubleVec::zero();
...
template <class MaskGatherChooser>
static vcp_inline
void vcp_simd_load_add_store(double * const addr, size_t offset, const DoubleVec& value, const vcp_lookupOrMask_vec& lookupORforceMask){
	DoubleVec sum = MaskGatherChooser::load(addr, offset, lookupORforceMask);
	sum = sum + value;
...
template <class MaskGatherChooser>
static vcp_inline
void vcp_simd_load_sub_store(double * const addr, size_t offset, const DoubleVec& value, const vcp_lookupOrMask_vec& lookupORforceMask){
	DoubleVec sum = MaskGatherChooser::load(addr, offset, lookupORforceMask);
	sum = sum - value;
	MaskGatherChooser::store(addr, offset, sum, lookupORforceMask);
}
...
template <class MaskGatherChooser>
static vcp_inline
void vcp_simd_load_add_store_masked(double * const addr, size_t offset, const DoubleVec& value, const vcp_lookupOrMask_vec& lookupORforceMask, const MaskVec mask){
	DoubleVec sum = MaskGatherChooser::load(addr, offset, lookupORforceMask);
	sum = sum + value;
...
template <class MaskGatherChooser>
static vcp_inline
void vcp_simd_load_sub_store_masked(double * const addr, size_t offset, const DoubleVec& value, const vcp_lookupOrMask_vec& lookupORforceMask, const MaskVec mask){
	DoubleVec sum = MaskGatherChooser::load(addr, offset, lookupORforceMask);
	sum = sum - value;
	MaskGatherChooser::storeMasked(addr, offset, sum, lookupORforceMask, mask);
}
...
public:
	...
	inline static MaskVec GetForceMask(const DoubleVec& m_r2, const DoubleVec& rc2, MaskVec& j_mask) {
		MaskVec result = (m_r2 != DoubleVec::zero()) and j_mask;
		j_mask = MaskVec::ones();
...
static inline calcDistLookup (const size_t & i_center_idx, const size_t & soa2_num_centers, const double & cutoffRadiusSquare,
		...
		const DoubleVec & cutoffRadiusSquareD, size_t end_j, const DoubleVec m1_r_x, const DoubleVec m1_r_y, const DoubleVec m1_r_z) {
	...
	for (; j < end_j; j += VCP_VEC_SIZE) {
		const DoubleVec m2_r_x = DoubleVec::aligned_load(soa2_m_r_x + j);
		const DoubleVec m2_r_y = DoubleVec::aligned_load(soa2_m_r_y + j);
		const DoubleVec m2_r_z = DoubleVec::aligned_load(soa2_m_r_z + j);

		const DoubleVec m_dx = m1_r_x - m2_r_x;
		const DoubleVec m_dy = m1_r_y - m2_r_y;
		const DoubleVec m_dz = m1_r_z - m2_r_z;

		const DoubleVec m_r2 = DoubleVec::scal_prod(m_dx, m_dy, m_dz, m_dx, m_dy, m_dz);
	...
	if (remainderMask.movemask()) {
		const DoubleVec m2_r_x = DoubleVec::aligned_load_mask(soa2_m_r_x + j, remainderMask);
		const DoubleVec m2_r_y = DoubleVec::aligned_load_mask(soa2_m_r_y + j, remainderMask);
		const DoubleVec m2_r_z = DoubleVec::aligned_load_mask(soa2_m_r_z + j, remainderMask);
