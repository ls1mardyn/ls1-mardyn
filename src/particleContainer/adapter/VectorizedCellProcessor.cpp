...
VectorizedCellProcessor::VectorizedCellProcessor(Domain & domain,
	...
	Log::global_log->info() << "VectorizedLJCellProcessor: no vectorization." << std::endl;
#elif VLJCP_VEC_TYPE==VLJCP_VEC_SSE3
	Log::global_log->info() << "VectorizedLJCellProcessor: using SSE3." << std::endl;
#elif VLJCP_VEC_TYPE==VLJCP_VEC_AVX
	Log::global_log->info() << "VectorizedLJCellProcessor: using AVX." << std::endl;
#elif VLJCP_VEC_TYPE==VLJCP_VEC_MIC
	Log::global_log->info() << "VectorizedCellProcessor: Compiled for Intel MIC" << std :: endl;
#endif
...
void VectorizedCellProcessor::preprocessCell(ParticleCell & c) {
	...
	if (_center_dist_lookup.get_size() < ceil(static_cast<double>(nLJCenters) / 8.0)) {
		_center_dist_lookup.resize(ceil(static_cast<double>(nLJCenters) / 8.0));
	}
#else
...
//			this casting via void* is required for gcc
	...
	if (ForcePolicy :: DetectSingleCell()) {
		size_t i_center_idx = 0;
		assert(_center_dist_lookup.get_size() >= soa2._ljcenters_size);
		AlignedArray<double, 64> lookup;
		lookup.resize(soa2._num_ljcenters);
		for (size_t i = 0; i < soa1._num_molecules; ++i) {

			unsigned long compute_molecule = 0;
			for (size_t j = ForcePolicy :: DetectSingleCell() ? i_center_idx + 1 : 0; j < soa2._num_ljcenters; ++j) {
				const double m_dx = soa1._mol_pos_x[i] - soa2._m_r_x[j];
				const double m_dy = soa1._mol_pos_y[i] - soa2._m_r_y[j];
				const double m_dz = soa1._mol_pos_z[i] - soa2._m_r_z[j];
				const double m_r2 = m_dx * m_dx + m_dy * m_dy + m_dz * m_dz;
			...
			for (int local_i = 0; local_i < soa1._mol_num_ljc[i]; local_i++ ) {
				for (size_t j = ForcePolicy :: DetectSingleCell() ? i_center_idx + 1 : 0; j < soa2._num_ljcenters; ++j) {
					_loopBodyNovec<CellPairPolicy_, MacroPolicy>(soa1, i_center_idx, soa2, j, &lookup[j]);
				}
				i_center_idx++;
			}
		}
	...
	double * const p_cfy1 = soa1._ljc_f_y;
	double * const p_cfz1 = soa1._ljc_f_z;
	const __int32 * const p_cid1 = soa1._ljc_id;
	...
	double * const p_cfy2 = soa2._ljc_f_y;
	double * const p_cfz2 = soa2._ljc_f_z;
	const __int32 * const p_cid2 = soa2._ljc_id;

	__mmask8* const p_center_dist_lookup = _center_dist_lookup;
	const size_t end_j = soa2._num_ljcenters & ~static_cast<size_t>(7);

	static const __mmask possibleRemainderMasks[8] = { 0x00, 0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3F, 0x7F  };
	const __mmask8 remainderMask = possibleRemainderMasks[soa2._num_ljcenters & static_cast<size_t>(7)];

	const __m512d one = _mm512_set1_pd(1.0);
	const __m512d rc2 = _mm512_extload_pd(&_rc2, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
	const __m512d zero = _mm512_setzero_pd();

	__m512d sum_upot = _mm512_setzero_pd();
	__m512d sum_virial = _mm512_setzero_pd();

	size_t i_center_idx = 0;
	assert(_center_dist_lookup.get_size() >= ceil(soa2._num_ljcenters / 8));

	for (size_t i = 0; i < soa1._num_molecules; ++i) {
		const __m512d m_r_x1 = _mm512_extload_pd(p_mol_rx1 + i, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
		const __m512d m_r_y1 = _mm512_extload_pd(p_mol_ry1 + i, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
		const __m512d m_r_z1 = _mm512_extload_pd(p_mol_rz1 + i, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);

		// distance and forca mask computation
		__mmask8 compute_molecule = 0x00;
		size_t j = ForcePolicy :: InitJ(i_center_idx);
		__mmask8 initJ_mask = ForcePolicy :: InitJ_Mask(i_center_idx);

		const __mmask8 remainderMask_Inner = ForcePolicy :: DetectSingleCell() and (soa2._num_ljcenters - j) < 8 ?
			initJ_mask & remainderMask :
			remainderMask
		;

		for (; j < end_j; j += 8) {
			const __m512d m_r_x2 = _mm512_load_pd(p_mrx2 + j);
			const __m512d m_r_y2 = _mm512_load_pd(p_mry2 + j);
			const __m512d m_r_z2 = _mm512_load_pd(p_mrz2 + j);

			const __m512d m_dx = _mm512_sub_pd(m_r_x1, m_r_x2);
			const __m512d m_dy = _mm512_sub_pd(m_r_y1, m_r_y2);
			const __m512d m_dz = _mm512_sub_pd(m_r_z1, m_r_z2);

			const __m512d m_dxdx = _mm512_mul_pd(m_dx, m_dx);
			const __m512d m_dydy = _mm512_mul_pd(m_dy, m_dy);
			const __m512d m_dzdz = _mm512_mul_pd(m_dz, m_dz);

			const __m512d m_dxdx_dydy = _mm512_add_pd(m_dxdx, m_dydy);

			const __m512d m_r2 = _mm512_add_pd(m_dxdx_dydy, m_dzdz);
			const __mmask8 forceMask = ForcePolicy :: GetForceMask(m_r2, rc2, initJ_mask);

			p_center_dist_lookup[j >> 3] = forceMask;
			compute_molecule |= forceMask;
		}
		if (remainderMask_Inner != 0x00) {
			const __m512d m_r_x2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_mrx2 + j);
			const __m512d m_r_y2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_mry2 + j);
			const __m512d m_r_z2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_mrz2 + j);
			...
			const __m512d m_r2 = _mm512_add_pd(m_dxdx_dydy, m_dzdz);
			const __mmask8 forceMask = remainderMask_Inner & ForcePolicy :: GetForceMask(m_r2, rc2, initJ_mask);

			p_center_dist_lookup[j >> 3] = forceMask;
			compute_molecule |= forceMask;
		}

		if (compute_molecule == 0x00) {
			i_center_idx += soa1._mol_num_ljc[i];
			continue;
		}
		// actual force computation
		for (int local_i = 0; local_i < soa1._mol_num_ljc[i]; local_i++ ) {
			__m512d sum_fx1 = zero;
			__m512d sum_fy1 = zero;
			__m512d sum_fz1 = zero;

			const __m512d c_r_x1 = _mm512_extload_pd(p_crx1 + i_center_idx, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
			const __m512d c_r_y1 = _mm512_extload_pd(p_cry1 + i_center_idx, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
			const __m512d c_r_z1 = _mm512_extload_pd(p_crz1 + i_center_idx, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);

			const double* const epsilonBase = &(_eps_sig[p_cid1[i_center_idx]][0]);
			const double* const sigmaBase = &(_eps_sig[p_cid1[i_center_idx]][1]);
			const double* const shift6Base = &(_shift6[p_cid1[i_center_idx]][0]);

			// Iterate over each pair of centers in the second cell.
			size_t j = ForcePolicy :: InitJ(i_center_idx);
			for (; j < end_j; j += 8) {
				const __mmask8 forceMask = p_center_dist_lookup[j >> 3];
				if (forceMask != 0x00) {
					const __m512d c_r_x2 = _mm512_load_pd(p_crx2 + j);
					const __m512d c_dx = _mm512_sub_pd(c_r_x1, c_r_x2);

					const __m512d c_r_y2 = _mm512_load_pd(p_cry2 + j);
					const __m512d c_dy = _mm512_sub_pd(c_r_y1, c_r_y2);

					const __m512d c_r_z2 = _mm512_load_pd(p_crz2 + j);
					const __m512d c_dz = _mm512_sub_pd(c_r_z1, c_r_z2);

					const __m512d c_dxdx = _mm512_mul_pd(c_dx, c_dx);
					const __m512d c_dydy = _mm512_mul_pd(c_dy, c_dy);
					const __m512d c_dzdz = _mm512_mul_pd(c_dz, c_dz);

					const __m512d c_dxdx_dydy = _mm512_add_pd(c_dxdx, c_dydy);
					const __m512d c_r2 = _mm512_add_pd(c_dxdx_dydy, c_dzdz);

					const __m512d r2_inv = _mm512_mask_div_pd(zero, forceMask, one, c_r2);

					const __m512i ids_j = _mm512_loadunpackhi_epi32(
						_mm512_loadunpacklo_epi32(_mm512_setzero_epi32(), p_cid2 + j),
						p_cid2 + j + (64 / sizeof(unsigned int))
					);
					const __m512i ids_j2 = _mm512_add_epi32(ids_j, ids_j);
					const __m512d eps_24 = _mm512_mask_i32logather_pd(zero, forceMask, ids_j2, epsilonBase, sizeof(double));
					const __m512d sig2 = _mm512_mask_i32logather_pd(zero, forceMask, ids_j2, sigmaBase, sizeof(double));

					const __m512d lj2 = _mm512_mul_pd(sig2, r2_inv);
					const __m512d lj4 = _mm512_mul_pd(lj2, lj2);
					const __m512d lj6 = _mm512_mul_pd(lj4, lj2);
					const __m512d lj12 = _mm512_mul_pd(lj6, lj6);
					const __m512d lj12m6 = _mm512_sub_pd(lj12, lj6);

					const __m512d eps24r2inv = _mm512_mul_pd(eps_24, r2_inv);
					const __m512d lj12lj12m6 = _mm512_add_pd(lj12, lj12m6);
					const __m512d scale = _mm512_mul_pd(eps24r2inv, lj12lj12m6);

					const __m512d fx = _mm512_mul_pd(c_dx, scale);
					const __m512d fy = _mm512_mul_pd(c_dy, scale);
					const __m512d fz = _mm512_mul_pd(c_dz, scale);
					...
					const __mmask8 macroMask = forceMask & MacroPolicy :: GetMacroMask(forceMask, m_dx, m_dy, m_dz);

					if (macroMask != 0x00) {
						const __m512d shift6 = _mm512_mask_i32logather_pd(zero, macroMask, ids_j, shift6Base, sizeof(double));

						const __m512d upot = _mm512_mul_pd(eps_24, lj12m6);
						const __m512d upot_masked = _mm512_mask_add_pd(zero, macroMask, shift6, upot);

						sum_upot = _mm512_add_pd(sum_upot, upot_masked);

						const __m512d vir_x = _mm512_mul_pd(m_dx, fx);
						const __m512d vir_y = _mm512_mul_pd(m_dy, fy);
						const __m512d vir_z = _mm512_mul_pd(m_dz, fz);

						const __m512d vir_xy = _mm512_add_pd(vir_x, vir_y);
						const __m512d vir_masked = _mm512_mask_add_pd(zero, macroMask, vir_xy, vir_z);

						sum_virial = _mm512_add_pd(sum_virial, vir_masked);
					}

					const __m512d old_fx2 = _mm512_load_pd(p_cfx2 + j);
					const __m512d new_fx2 = _mm512_sub_pd(old_fx2, fx);
					_mm512_store_pd(p_cfx2 + j, new_fx2);

					const __m512d old_fy2 = _mm512_load_pd(p_cfy2 + j);
					const __m512d new_fy2 = _mm512_sub_pd(old_fy2, fy);
					_mm512_store_pd(p_cfy2 + j, new_fy2);

					const __m512d old_fz2 = _mm512_load_pd(p_cfz2 + j);
					const __m512d new_fz2 = _mm512_sub_pd(old_fz2, fz);
					_mm512_store_pd(p_cfz2 + j, new_fz2);

					sum_fx1 = _mm512_add_pd(sum_fx1, fx);
					sum_fy1 = _mm512_add_pd(sum_fy1, fy);
					sum_fz1 = _mm512_add_pd(sum_fz1, fz);
				}
			}
			if (remainderMask_Inner != 0x00) {
				const __mmask8 forceMask = p_center_dist_lookup[j >> 3] & remainderMask_Inner;
				if (forceMask != 0x00) {
					const __m512d c_r_x2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_crx2 + j);
					const __m512d c_dx = _mm512_sub_pd(c_r_x1, c_r_x2);

					const __m512d c_r_y2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_cry2 + j);
					const __m512d c_dy = _mm512_sub_pd(c_r_y1, c_r_y2);

					const __m512d c_r_z2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_crz2 + j);
					const __m512d c_dz = _mm512_sub_pd(c_r_z1, c_r_z2);
					...
					const __m512i ids_j = _mm512_mask_loadunpackhi_epi32(
						_mm512_mask_loadunpacklo_epi32(_mm512_setzero_epi32(), remainderMask_Inner, p_cid2 + j),
						remainderMask_Inner,
						p_cid2 + j + (64 / sizeof(unsigned int))
					);
					const __m512i ids_j2 = _mm512_add_epi32(ids_j, ids_j);
					...
					const __mmask8 macroMask = remainderMask_Inner & MacroPolicy :: GetMacroMask(forceMask, m_dx, m_dy, m_dz);
					...
					const __m512d old_fx2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_cfx2 + j);
					const __m512d new_fx2 = _mm512_sub_pd(old_fx2, fx);
					_mm512_mask_store_pd(p_cfx2 + j, remainderMask_Inner, new_fx2);

					const __m512d old_fy2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_cfy2 + j);
					const __m512d new_fy2 = _mm512_sub_pd(old_fy2, fy);
					_mm512_mask_store_pd(p_cfy2 + j, remainderMask_Inner, new_fy2);

					const __m512d old_fz2 = _mm512_mask_load_pd(zero, remainderMask_Inner, p_cfz2 + j);
					const __m512d new_fz2 = _mm512_sub_pd(old_fz2, fz);
					_mm512_mask_store_pd(p_cfz2 + j, remainderMask_Inner, new_fz2);
			...
			p_cfx1[i_center_idx] += _mm512_reduce_add_pd(sum_fx1);
			p_cfy1[i_center_idx] += _mm512_reduce_add_pd(sum_fy1);
			p_cfz1[i_center_idx] += _mm512_reduce_add_pd(sum_fz1);

			++i_center_idx;
		}
	}

	_upot6lj += _mm512_reduce_add_pd(sum_upot);
	_virial += _mm512_reduce_add_pd(sum_virial);
#endif
