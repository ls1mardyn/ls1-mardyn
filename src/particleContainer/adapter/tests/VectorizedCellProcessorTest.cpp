...
void VectorizedCellProcessorTest::testLennardJonesVectorization() {
	...
	for (ParticleIterator m = container_1->iterator(); m.isValid(); ++m) {
		m->calcFM();
	}
	...
	for (ParticleIterator m = container_2->iterator(); m.isValid(); ++m) {
		m->calcFM();
	}
	...
	for (ParticleIterator m_2 = container_2->iterator(); m_2.isValid(); ++m_2) {
		for (int i = 0; i < 3; i++) {
			std::stringstream str;
