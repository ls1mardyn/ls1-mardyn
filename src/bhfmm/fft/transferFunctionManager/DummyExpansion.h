...
class DummyStorage {
  public:
    ...
    /* destructor */
    ~DummyStorage() {delete[] entries;}

    inline double & access(unsigned i, unsigned j) {return entries[index(i,j)];}
    inline const double & access_const(unsigned i, unsigned j) const {return entries[index(i,j)];}
...
class DummyExpansion : public FFTAccelerableExpansion {
  public:
    ...
    double & get_C(unsigned l, unsigned m) {return acc_C(l,m);}
    double & get_S(unsigned l, unsigned m) {return acc_S(l,m);}

  private:
    //direct accessor
    inline double & acc_C(unsigned l, unsigned m) {return C.access(l,m);}
    inline double & acc_S(unsigned l, unsigned m) {return S.access(l,m);}

    //const accesssor
    inline const double acc_c_C(unsigned l, unsigned m) const {return C.access_const(l,m);}
    inline const double acc_c_S(unsigned l, unsigned m) const {return S.access_const(l,m);}
