...
TransferFunctionManager_UniformGrid::TransferFunctionManager_UniformGrid(int ord, FFTAccelerationAPI* FFTA, bool verbose) 
...
TransferFunctionManager_UniformGrid::~TransferFunctionManager_UniformGrid() {
  if(_verbose)
    std::cout << "TransferFunctionManager_UniformGrid's stats: TF asked=" << _asked << " | TF builded=" << _builded << std::endl;
...
void TransferFunctionManager_UniformGrid::buildTransferFunction() {
  ...
  for(i=-3;i<4;i++) {
    ...
    for(j=-3;j<4;j++) {
      ...
      for(k=-3;k<4;k++) {
        if(std::max(abs(i),std::max(abs(j),abs(k))) < 2)  continue; //skip useless tf (cells too close)
        
        z = ((double)k) * base_unit;
        _storage[i+3][j+3][k+3] = tfman.getTransferFunction(x,y,z,base_unit,base_unit,base_unit); //Use general class
        _builded++;
      }
    }
  }
...
FFTDataContainer* TransferFunctionManager_UniformGrid::getTransferFunction(int x, int y, int z, double cell_size_x, double cell_size_y, double cell_size_z){
  _asked++;
