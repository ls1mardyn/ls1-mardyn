...
void FFTSettings::setOptions(string option)
{
  if(option == "off") {
    ...
    FFTSettings::USE_FFT = true; FFTSettings::USE_FFTW = false; 
  } else if(option == "fftw") {
    FFTSettings::USE_FFT = true; FFTSettings::USE_FFTW = true; 
    ...
    FFTSettings::USE_BLOCK = true; FFTSettings::USE_ADVBLOCK = false;
  } else if(option == "advblock") {
    FFTSettings::USE_BLOCK = true; FFTSettings::USE_ADVBLOCK = true;
  } else if(option == "noblock") {
    FFTSettings::USE_BLOCK = false; FFTSettings::USE_ADVBLOCK = false;
  } else if(option == "ORed") {
    FFTSettings::USE_ORDER_REDUCTION = true;
...
vector<string> FFTSettings::getAvailableOptions()
{
  ...
  options.push_back("TFMem:   enable TransferFunction precomputation (requires uniform grid)");
  options.push_back("noTFMem: disable TransferFunction precomputation");
  options.push_back("vec:     enable vectorization (must have been compiled with vec support, see FFTSettings_preprocessor.h)");
  options.push_back("novec:   disable vectorization");
  options.push_back("2way:    enable 2way M2L (better on non uniform grid), requires vec");
  ...
  options.push_back("ORed:    enable order reduction (block and advblock only)");
...
void FFTSettings::printCurrentOptions()
{
  if(FFTSettings::USE_FFT){
    ...
    if(FFTSettings::USE_BLOCK) {
      if(FFTSettings::USE_ADVBLOCK) printf("Adv. Block FFT enabled\n");
      else printf("Block FFT enabled\n");
    }
      
    if(FFTSettings::USE_TFMANAGER_UNIFORMGRID)
      printf("TransferFunctionManager_UniformGrid (precomputation of TF, requires an uniform grid)\n");
    else
