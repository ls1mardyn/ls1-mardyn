...
class FFTW_API {
  ...
  public:
     ...
     * @param int ny
     */
    FFTW_API(int nx, int ny) : _nx(nx), _ny(ny)
    {
      ...
      //if(ny>1) {
        _forward  = fftw_plan_dft_2d(nx, ny, _f_in, _f_out, FFTW_FORWARD,  FFTW_ESTIMATE);
        _backward = fftw_plan_dft_2d(nx, ny, _b_in, _b_out, FFTW_BACKWARD, FFTW_ESTIMATE);
      //} else {
...
#else
    fftw_complex* getIn_Forward()  {return _f_in;}
    fftw_complex* getIn_Backward() {return _b_in;}
    
    fftw_complex* FFTAndGetOutput_Forward()  {fftw_execute(_forward);  return _f_out;}
    fftw_complex* FFTAndGetOutput_Backward() {fftw_execute(_backward); return _b_out;}
#endif    
