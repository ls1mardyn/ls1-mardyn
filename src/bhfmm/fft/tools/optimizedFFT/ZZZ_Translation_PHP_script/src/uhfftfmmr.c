...
void	MFFTR12(Complex *x)
{
	...
	tmp9i = -(Im(x[4])-Im(x[2]));
	tmp9r = Re(x[4])+Re(x[2]);
	tmp10i = -(Im(x[4])+Im(x[2]));
	tmp10r = Re(x[4])-Re(x[2]);
	tmp11i = Im(x[5])+Im(x[1]);
	tmp11r = Re(x[5])-Re(x[1]);
	tmp12i = Im(x[5])-Im(x[1]);
	tmp12r = Re(x[5])+Re(x[1]);
	tmp13i = tmp9i+tmp11i;
	tmp13r = tmp9r+tmp11r;
	tmp14i = tmp9i-tmp11i;
	tmp14r = tmp9r-tmp11r;
	...
	tmp20r = C6*tmp18i;
	tmp21i = tmp19i+tmp0i;
	tmp21r = tmp19r+Re(x[0]);
	tmp22i = tmp21i+tmp20i;
	tmp22r = tmp21r+tmp20r;
	tmp24i = tmp0i+tmp17i;
	tmp24r = Re(x[0])+tmp17r;
	tmp25i = tmp8i+tmp16i;
	tmp25r = tmp8r+tmp16r;
	tmp26i = tmp8i-tmp16i;
	tmp26r = tmp8r-tmp16r;
	...
	tmp36r = C6*tmp34i;
	tmp37i = tmp35i-tmp0i;
	tmp37r = tmp35r+Re(x[0]);
	tmp39i = tmp37i-tmp36i;
	tmp39r = tmp37r-tmp36r;
	tmp41i = tmp7i+tmp15i;
	tmp41r = tmp7r+tmp15r;
	tmp48i = -(tmp0r-tmp41i);
	tmp48r = Re(x[0])+tmp41r;
	Re(x[0]) = tmp24r;
	Im(x[0]) = tmp24i;
	Re(x[1]) = tmp30r;
	Im(x[1]) = tmp30i;
...
void	MFFTR14(Complex *x)
{
	...
	tmp2r = C16*Re(x[2]);
	tmp2i = C16*Im(x[2]);
	tmp3r = C9*tmp0r-C11*tmp1r-C13*tmp2r+Re(x[0]);
	tmp4r = tmp0r+tmp1r;
	tmp5r = -C11*tmp0r-C13*tmp1r+C9*tmp2r+Re(x[0]);
	tmp6r = tmp4r+tmp2r;
	tmp7r = -C13*tmp0r+C9*tmp1r-C11*tmp2r+Re(x[0]);
...
void	MIFFTR14(Complex *x)
{
	...
	tmp20r = Re(x[0])+tmp9r;
	tmp21i = Im(x[1])-Im(x[6]);
	tmp21r = Re(x[1])+Re(x[6]);
	tmp22i = Im(x[1])+Im(x[6]);
	tmp22r = Re(x[1])-Re(x[6]);
	tmp23i = Im(x[3])-Im(x[4]);
	tmp23r = Re(x[3])+Re(x[4]);
	tmp24i = Im(x[3])+Im(x[4]);
	tmp24r = Re(x[3])-Re(x[4]);
	tmp25i = -(Im(x[2])-Im(x[5]));
	tmp25r = Re(x[2])+Re(x[5]);
	tmp26i = -(Im(x[2])+Im(x[5]));
	tmp26r = Re(x[2])-Re(x[5]);
	tmp27i = C9*tmp21i-C11*tmp23i-C13*tmp25i-Im(x[0]);
	tmp27r = C9*tmp21r-C11*tmp23r-C13*tmp25r+Re(x[0]);
	tmp28r = tmp21r+tmp23r;
	tmp29i = -C11*tmp21i-C13*tmp23i+C9*tmp25i-Im(x[0]);
	tmp29r = -C11*tmp21r-C13*tmp23r+C9*tmp25r+Re(x[0]);
	tmp30r = tmp28r+tmp25r;
	tmp31i = -C13*tmp21i+C9*tmp23i-C11*tmp25i-Im(x[0]);
	tmp31r = -C13*tmp21r+C9*tmp23r-C11*tmp25r+Re(x[0]);
...
void	MFFTR16(Complex *x)
{
	...
	tmp21r = tmp4r+tmp14r;
	tmp22r = tmp4r-tmp14r;
	tmp11i = Im(x[1])+Im(x[7]);
	tmp23r = Re(x[1])-Re(x[7]);
	tmp12i = Im(x[1])-Im(x[7]);
	tmp24r = Re(x[1])+Re(x[7]);
	tmp13i = Im(x[5])+Im(x[3]);
	tmp25r = Re(x[5])-Re(x[3]);
	tmp14i = Im(x[5])-Im(x[3]);
	tmp26r = Re(x[5])+Re(x[3]);
	...
	tmp23i = Im(x[3])+Im(x[5]);
	tmp35r = Re(x[3])-Re(x[5]);
	tmp24i = Im(x[3])-Im(x[5]);
	tmp36r = Re(x[3])+Re(x[5]);
	tmp25i = Im(x[7])+Im(x[1]);
	tmp37r = Re(x[7])-Re(x[1]);
	tmp26i = Im(x[7])-Im(x[1]);
	tmp38r = Re(x[7])+Re(x[1]);
...
void	MFFTR18(Complex *x)
{
	...
	const REAL	C9 =     0.17364817766693;	/* REALCONST	*/
	const REAL	C12 =     0.34202014332567;	/* REALCONST	*/
	const REAL	C1 =                  0.5;	/* REALCONST	*/
	const REAL	C8 =     0.64278760968654;	/* REALCONST	*/
	const REAL	C7 =     0.76604444311898;	/* REALCONST	*/
	const REAL	C6 =     0.86602540378444;	/* REALCONST	*/
	const REAL	C11 =     0.93969262078591;	/* REALCONST	*/
	const REAL	C10 =     0.98480775301221;	/* REALCONST	*/
	const REAL	C5 =                    2;	/* INTEGERCONST	*/
	...
	tmp14r = Re(x[8])+tmp7r;
	tmp9i = -(Im(x[4])-Im(x[8]));
	tmp15r = Re(x[4])+Re(x[8]);
	tmp10i = -(Im(x[4])+Im(x[8]));
	tmp16r = Re(x[4])-Re(x[8]);
	tmp11i = -C1*tmp9i;
	tmp17r = -C1*tmp15r;
	tmp12i = -C6*tmp16r;
	tmp18r = C6*tmp10i;
	tmp13i = tmp11i+Im(x[2]);
	tmp19r = tmp17r+Re(x[2]);
	tmp14i = tmp13i+tmp12i;
	tmp20r = tmp19r+tmp18r;
	tmp15i = tmp13i-tmp12i;
	tmp21r = tmp19r-tmp18r;
	tmp16i = Im(x[2])+tmp9i;
	tmp22r = Re(x[2])+tmp15r;
	...
	tmp44i = Im(x[5])+Im(x[7]);
	tmp52r = -(Re(x[5])-Re(x[7]));
	tmp45i = Im(x[5])-Im(x[7]);
	tmp53r = -(Re(x[5])+Re(x[7]));
	tmp46i = -C1*tmp44i;
	tmp54r = -C1*tmp52r;
	tmp47i = -C6*tmp53r;
	tmp55r = C6*tmp45i;
	tmp48i = tmp46i+Im(x[1]);
	tmp56r = tmp54r+Re(x[1]);
	tmp49i = tmp48i+tmp47i;
	tmp57r = tmp56r+tmp55r;
	tmp50i = tmp48i-tmp47i;
	tmp58r = tmp56r-tmp55r;
	tmp51i = Im(x[1])+tmp44i;
	tmp59r = Re(x[1])+tmp52r;
	tmp52i = Im(x[5])+Im(x[1]);
	tmp60r = Re(x[5])-Re(x[1]);
	tmp53i = Im(x[5])-Im(x[1]);
	tmp61r = Re(x[5])+Re(x[1]);
	tmp54i = -C1*tmp52i;
	tmp62r = -C1*tmp60r;
	tmp55i = -C6*tmp61r;
	tmp63r = C6*tmp53i;
	tmp56i = tmp54i+Im(x[7]);
	tmp64r = tmp62r-Re(x[7]);
	tmp57i = tmp56i+tmp55i;
	tmp65r = tmp64r+tmp63r;
	tmp58i = tmp56i-tmp55i;
	tmp66r = tmp64r-tmp63r;
	tmp59i = Im(x[7])+tmp52i;
	tmp67r = -(Re(x[7])-tmp60r);
...
void	MIFFTR18(Complex *x)
{
	...
	tmp11i = C6*tmp9r;
	tmp11r = -C6*tmp9i;
	tmp12i = tmp10i-Im(x[1]);
	tmp12r = tmp10r+Re(x[1]);
	...
	tmp15i = -(Im(x[1])-tmp8i);
	tmp15r = Re(x[1])+tmp8r;
	tmp16i = -(Im(x[5])-Im(x[8]));
	tmp16r = Re(x[5])+Re(x[8]);
	tmp17i = -(Im(x[5])+Im(x[8]));
	tmp17r = Re(x[5])-Re(x[8]);
	tmp18i = -C1*tmp16i;
	tmp18r = -C1*tmp16r;
	tmp19i = C6*tmp17r;
	tmp19r = -C6*tmp17i;
	tmp20i = tmp18i+Im(x[2]);
	tmp20r = tmp18r+Re(x[2]);
	tmp21i = tmp20i+tmp19i;
	tmp21r = tmp20r+tmp19r;
	tmp22i = tmp20i-tmp19i;
	tmp22r = tmp20r-tmp19r;
	tmp23i = Im(x[2])+tmp16i;
	tmp23r = Re(x[2])+tmp16r;
	...
	tmp51i = tmp6i+tmp44i;
	tmp51r = tmp6r+tmp44r;
	tmp52i = Im(x[3])-Im(x[6]);
	tmp52r = Re(x[3])+Re(x[6]);
	tmp53i = Im(x[3])+Im(x[6]);
	tmp53r = Re(x[3])-Re(x[6]);
	tmp54i = -C1*tmp52i;
	tmp54r = -C1*tmp52r;
	tmp55i = C6*tmp53r;
	tmp55r = -C6*tmp53i;
	tmp56i = tmp54i-Im(x[0]);
	tmp56r = tmp54r+Re(x[0]);
	tmp57i = tmp56i+tmp55i;
	tmp57r = tmp56r+tmp55r;
	tmp58i = tmp56i-tmp55i;
	tmp58r = tmp56r-tmp55r;
	tmp59i = -(Im(x[0])-tmp52i);
	tmp59r = Re(x[0])+tmp52r;
	tmp60i = -(Im(x[4])-Im(x[7]));
	tmp60r = Re(x[4])+Re(x[7]);
	tmp61i = -(Im(x[4])+Im(x[7]));
	tmp61r = Re(x[4])-Re(x[7]);
	tmp62i = -C1*tmp60i;
	tmp62r = -C1*tmp60r;
	tmp63i = C6*tmp61r;
	tmp63r = -C6*tmp61i;
	tmp64i = tmp62i+Im(x[1]);
	tmp64r = tmp62r+Re(x[1]);
	tmp65i = tmp64i+tmp63i;
	tmp65r = tmp64r+tmp63r;
	tmp66i = tmp64i-tmp63i;
	tmp66r = tmp64r-tmp63r;
	tmp67i = Im(x[1])+tmp60i;
	tmp67r = Re(x[1])+tmp60r;
	tmp68i = Im(x[5])-Im(x[8]);
	tmp68r = Re(x[5])+Re(x[8]);
	tmp69i = Im(x[5])+Im(x[8]);
	tmp69r = Re(x[5])-Re(x[8]);
	tmp70i = -C1*tmp68i;
	tmp70r = -C1*tmp68r;
	tmp71i = C6*tmp69r;
	tmp71r = -C6*tmp69i;
	tmp72i = tmp70i-Im(x[2]);
	tmp72r = tmp70r+Re(x[2]);
	tmp73i = tmp72i+tmp71i;
	tmp73r = tmp72r+tmp71r;
	tmp74i = tmp72i-tmp71i;
	tmp74r = tmp72r-tmp71r;
	tmp75i = -(Im(x[2])-tmp68i);
	tmp75r = Re(x[2])+tmp68r;
...
void	MFFTR20(Complex *x)
{
	...
	tmp15r = Re(x[0])+tmp2r;
	tmp2i = Im(x[1])+Im(x[9]);
	tmp16r = Re(x[1])+Re(x[9]);
	tmp3i = Im(x[1])-Im(x[9]);
	tmp17r = Re(x[1])-Re(x[9]);
	tmp4i = Im(x[3])+Im(x[7]);
	tmp18r = -(Re(x[3])+Re(x[7]));
	tmp5i = Im(x[3])-Im(x[7]);
	tmp19r = -(Re(x[3])-Re(x[7]));
	tmp6i = tmp2i+tmp4i;
	...
	tmp22i = Im(x[9])+Im(x[1]);
	tmp48r = -(Re(x[9])+Re(x[1]));
	tmp23i = Im(x[9])-Im(x[1]);
	tmp49r = -(Re(x[9])-Re(x[1]));
	tmp24i = Im(x[7])+Im(x[3]);
	tmp50r = Re(x[7])+Re(x[3]);
	tmp25i = Im(x[7])-Im(x[3]);
	tmp51r = Re(x[7])-Re(x[3]);
	...
	tmp29i = C15*tmp27i;
	tmp55r = C15*tmp53r;
	tmp30i = tmp28i+Im(x[5]);
	tmp56r = tmp54r-Re(x[5]);
	...
	tmp39i = Im(x[5])+tmp26i;
	tmp65r = -(Re(x[5])-tmp52r);
...
void	MIFFTR20(Complex *x)
{
	...
	tmp17i = Im(x[0])+tmp4i;
	tmp17r = Re(x[0])+tmp4r;
	tmp18i = Im(x[1])+Im(x[9]);
	tmp18r = Re(x[1])+Re(x[9]);
	tmp19i = Im(x[1])-Im(x[9]);
	tmp19r = Re(x[1])-Re(x[9]);
	tmp20i = -(Im(x[7])+Im(x[3]));
	tmp20r = Re(x[7])+Re(x[3]);
	tmp21i = -(Im(x[7])-Im(x[3]));
	tmp21r = Re(x[7])-Re(x[3]);
	...
	tmp25i = C15*tmp23i;
	tmp25r = C15*tmp23r;
	tmp26i = tmp24i+Im(x[5]);
	tmp26r = tmp24r+Re(x[5]);
	...
	tmp35i = Im(x[5])+tmp22i;
	tmp35r = Re(x[5])+tmp22r;
	tmp36i = Im(x[6])-Im(x[4]);
	tmp36r = Re(x[6])+Re(x[4]);
	tmp37i = Im(x[6])+Im(x[4]);
	tmp37r = Re(x[6])-Re(x[4]);
	tmp38i = Im(x[2])-Im(x[8]);
	tmp38r = Re(x[2])+Re(x[8]);
	tmp39i = Im(x[2])+Im(x[8]);
	tmp39r = Re(x[2])-Re(x[8]);
	...
	tmp43i = C15*tmp41i;
	tmp43r = C15*tmp41r;
	tmp44i = tmp42i-Im(x[0]);
	tmp44r = tmp42r+Re(x[0]);
	...
	tmp53i = -(Im(x[0])-tmp40i);
	tmp53r = Re(x[0])+tmp40r;
	tmp54i = -(Im(x[1])+Im(x[9]));
	tmp54r = Re(x[1])+Re(x[9]);
	tmp55i = -(Im(x[1])-Im(x[9]));
	tmp55r = Re(x[1])-Re(x[9]);
	tmp56i = Im(x[7])+Im(x[3]);
	tmp56r = Re(x[7])+Re(x[3]);
	tmp57i = Im(x[7])-Im(x[3]);
	tmp57r = Re(x[7])-Re(x[3]);
	...
	tmp61i = C15*tmp59i;
	tmp61r = C15*tmp59r;
	tmp62i = tmp60i-Im(x[5]);
	tmp62r = tmp60r+Re(x[5]);
	...
	tmp71i = -(Im(x[5])-tmp58i);
	tmp71r = Re(x[5])+tmp58r;
...
void	MFFTR22(Complex *x)
{
	...
	tmp4r = C24*Re(x[6]);
	tmp4i = -C24*Im(x[6]);
	tmp5r = C13*tmp0r+C15*tmp1r-C17*tmp2r-C19*tmp3r-C21*tmp4r+Re(x[0]);
	tmp6r = tmp0r+tmp1r;
	tmp7r = C15*tmp0r-C17*tmp1r-C19*tmp2r-C21*tmp3r+C13*tmp4r+Re(x[0]);
	tmp8r = tmp6r+tmp2r;
	tmp9r = -C17*tmp0r-C19*tmp1r-C21*tmp2r+C13*tmp3r+C15*tmp4r+Re(x[0]);
	tmp10r = tmp8r+tmp3r;
	tmp11r = -C19*tmp0r-C21*tmp1r+C13*tmp2r+C15*tmp3r-C17*tmp4r+Re(x[0]);
	tmp12r = tmp10r+tmp4r;
	tmp13r = -C21*tmp0r+C13*tmp1r+C15*tmp2r-C17*tmp3r-C19*tmp4r+Re(x[0]);
...
void	MIFFTR22(Complex *x)
{
	...
	tmp0i = -(Im(x[1])-Im(x[10]));
	tmp0r = Re(x[1])+Re(x[10]);
	tmp1i = -(Im(x[1])+Im(x[10]));
	tmp1r = Re(x[1])-Re(x[10]);
	tmp2i = Im(x[2])-Im(x[9]);
	tmp2r = Re(x[2])+Re(x[9]);
	tmp3i = Im(x[2])+Im(x[9]);
	tmp3r = Re(x[2])-Re(x[9]);
	...
	tmp35i = Im(x[1])-Im(x[10]);
	tmp35r = Re(x[1])+Re(x[10]);
	tmp36i = Im(x[1])+Im(x[10]);
	tmp36r = Re(x[1])-Re(x[10]);
	tmp37i = -(Im(x[2])-Im(x[9]));
	tmp37r = Re(x[2])+Re(x[9]);
	tmp38i = -(Im(x[2])+Im(x[9]));
	tmp38r = Re(x[2])-Re(x[9]);
	tmp39i = -(Im(x[4])-Im(x[7]));
	tmp39r = Re(x[4])+Re(x[7]);
	tmp40i = -(Im(x[4])+Im(x[7]));
	tmp40r = Re(x[4])-Re(x[7]);
	tmp41i = -(Im(x[8])-Im(x[3]));
	tmp41r = Re(x[8])+Re(x[3]);
	tmp42i = -(Im(x[8])+Im(x[3]));
	tmp42r = Re(x[8])-Re(x[3]);
	tmp43i = Im(x[5])-Im(x[6]);
	tmp43r = Re(x[5])+Re(x[6]);
	tmp44i = Im(x[5])+Im(x[6]);
	tmp44r = Re(x[5])-Re(x[6]);
	tmp45i = C13*tmp35i+C15*tmp37i-C17*tmp39i-C19*tmp41i-C21*tmp43i-Im(x[0]);
	tmp45r = C13*tmp35r+C15*tmp37r-C17*tmp39r-C19*tmp41r-C21*tmp43r+Re(x[0]);
	tmp46r = tmp35r+tmp37r;
	tmp47i = C15*tmp35i-C17*tmp37i-C19*tmp39i-C21*tmp41i+C13*tmp43i-Im(x[0]);
	tmp47r = C15*tmp35r-C17*tmp37r-C19*tmp39r-C21*tmp41r+C13*tmp43r+Re(x[0]);
	tmp48r = tmp46r+tmp39r;
	tmp49i = -C17*tmp35i-C19*tmp37i-C21*tmp39i+C13*tmp41i+C15*tmp43i-Im(x[0]);
	tmp49r = -C17*tmp35r-C19*tmp37r-C21*tmp39r+C13*tmp41r+C15*tmp43r+Re(x[0]);
	tmp50r = tmp48r+tmp41r;
	tmp51i = -C19*tmp35i-C21*tmp37i+C13*tmp39i+C15*tmp41i-C17*tmp43i-Im(x[0]);
	tmp51r = -C19*tmp35r-C21*tmp37r+C13*tmp39r+C15*tmp41r-C17*tmp43r+Re(x[0]);
	tmp52r = tmp50r+tmp43r;
	tmp53i = -C21*tmp35i+C13*tmp37i+C15*tmp39i-C17*tmp41i-C19*tmp43i-Im(x[0]);
	tmp53r = -C21*tmp35r+C13*tmp37r+C15*tmp39r-C17*tmp41r-C19*tmp43r+Re(x[0]);
...
void	MFFTR24(Complex *x)
{
	...
	const REAL	C4 =                  0.5;	/* REALCONST	*/
	const REAL	C3 =     0.70710678118655;	/* REALCONST	*/
	const REAL	C7 =     0.86602540378444;	/* REALCONST	*/
	const REAL	C2 =                    2;	/* INTEGERCONST	*/
	...
	tmp21r = tmp4r+tmp14r;
	tmp22r = tmp4r-tmp14r;
	tmp11i = -(Im(x[8])-Im(x[4]));
	tmp23r = Re(x[8])+Re(x[4]);
	tmp12i = -(Im(x[8])+Im(x[4]));
	tmp24r = Re(x[8])-Re(x[4]);
	tmp13i = Im(x[10])-Im(x[2]);
	tmp25r = Re(x[10])+Re(x[2]);
	tmp14i = Im(x[10])+Im(x[2]);
	tmp26r = Re(x[10])-Re(x[2]);
	tmp15i = tmp11i+tmp13i;
	tmp27r = tmp23r+tmp25r;
	tmp16i = tmp11i-tmp13i;
	tmp28r = tmp23r-tmp25r;
	...
	tmp18i = tmp12i+tmp26r;
	tmp30r = tmp24r-tmp14i;
	tmp19i = Im(x[1])+Im(x[11]);
	tmp31r = Re(x[1])-Re(x[11]);
	tmp20i = Im(x[1])-Im(x[11]);
	tmp32r = Re(x[1])+Re(x[11]);
	tmp21i = Im(x[5])+Im(x[7]);
	tmp33r = -(Re(x[5])-Re(x[7]));
	tmp22i = Im(x[5])-Im(x[7]);
	tmp34r = -(Re(x[5])+Re(x[7]));
	...
	tmp37i = Im(x[8])-Im(x[4]);
	tmp49r = Re(x[8])+Re(x[4]);
	tmp38i = Im(x[8])+Im(x[4]);
	tmp50r = Re(x[8])-Re(x[4]);
	tmp39i = Im(x[2])-Im(x[10]);
	tmp51r = Re(x[2])+Re(x[10]);
	tmp40i = Im(x[2])+Im(x[10]);
	tmp52r = Re(x[2])-Re(x[10]);
	...
	tmp45i = Im(x[7])+Im(x[5]);
	tmp57r = -(Re(x[7])-Re(x[5]));
	tmp46i = Im(x[7])-Im(x[5]);
	tmp58r = -(Re(x[7])+Re(x[5]));
	tmp47i = Im(x[11])+Im(x[1]);
	tmp59r = Re(x[11])-Re(x[1]);
	tmp48i = Im(x[11])-Im(x[1]);
	tmp60r = Re(x[11])+Re(x[1]);
	tmp49i = tmp45i+tmp47i;
	tmp61r = tmp57r+tmp59r;
	tmp50i = tmp45i-tmp47i;
	tmp62r = tmp57r-tmp59r;
...
void	MFFTR26(Complex *x)
{
	...
	tmp5r = C28*Re(x[6]);
	tmp5i = C28*Im(x[6]);
	tmp6r = C15*tmp0r+C17*tmp1r-C19*tmp2r-C21*tmp3r+C23*tmp4r-C25*tmp5r+Re(x[0]);
	tmp7r = tmp0r+tmp1r;
	tmp8r = C17*tmp0r-C19*tmp1r-C21*tmp2r+C23*tmp3r-C25*tmp4r+C15*tmp5r+Re(x[0]);
	tmp9r = tmp7r+tmp2r;
	tmp10r = -C19*tmp0r-C21*tmp1r+C23*tmp2r-C25*tmp3r+C15*tmp4r+C17*tmp5r+Re(x[0]);
	tmp11r = tmp9r+tmp3r;
	tmp12r = -C21*tmp0r+C23*tmp1r-C25*tmp2r+C15*tmp3r+C17*tmp4r-C19*tmp5r+Re(x[0]);
	tmp13r = tmp11r+tmp4r;
	tmp14r = C23*tmp0r-C25*tmp1r+C15*tmp2r+C17*tmp3r-C19*tmp4r-C21*tmp5r+Re(x[0]);
	tmp15r = tmp13r+tmp5r;
	tmp16r = -C25*tmp0r+C15*tmp1r+C17*tmp2r-C19*tmp3r-C21*tmp4r+C23*tmp5r+Re(x[0]);
...
void	MIFFTR26(Complex *x)
{
	...
	tmp0i = -(Im(x[1])-Im(x[12]));
	tmp0r = Re(x[1])+Re(x[12]);
	tmp1i = -(Im(x[1])+Im(x[12]));
	tmp1r = Re(x[1])-Re(x[12]);
	tmp2i = Im(x[2])-Im(x[11]);
	tmp2r = Re(x[2])+Re(x[11]);
	tmp3i = Im(x[2])+Im(x[11]);
	tmp3r = Re(x[2])-Re(x[11]);
	tmp4i = Im(x[4])-Im(x[9]);
	tmp4r = Re(x[4])+Re(x[9]);
	tmp5i = Im(x[4])+Im(x[9]);
	tmp5r = Re(x[4])-Re(x[9]);
	...
	tmp7i = Im(x[8])+Im(x[5]);
	tmp7r = Re(x[8])-Re(x[5]);
	tmp8i = -(Im(x[3])-Im(x[10]));
	tmp8r = Re(x[3])+Re(x[10]);
	tmp9i = -(Im(x[3])+Im(x[10]));
	tmp9r = Re(x[3])-Re(x[10]);
	tmp10i = Im(x[6])-Im(x[7]);
	tmp10r = Re(x[6])+Re(x[7]);
	tmp11i = Im(x[6])+Im(x[7]);
	tmp11r = Re(x[6])-Re(x[7]);
	...
	tmp42i = Im(x[1])-Im(x[12]);
	tmp42r = Re(x[1])+Re(x[12]);
	tmp43i = Im(x[1])+Im(x[12]);
	tmp43r = Re(x[1])-Re(x[12]);
	tmp44i = -(Im(x[2])-Im(x[11]));
	tmp44r = Re(x[2])+Re(x[11]);
	tmp45i = -(Im(x[2])+Im(x[11]));
	tmp45r = Re(x[2])-Re(x[11]);
	tmp46i = -(Im(x[4])-Im(x[9]));
	tmp46r = Re(x[4])+Re(x[9]);
	tmp47i = -(Im(x[4])+Im(x[9]));
	tmp47r = Re(x[4])-Re(x[9]);
	tmp48i = -(Im(x[8])-Im(x[5]));
	tmp48r = Re(x[8])+Re(x[5]);
	tmp49i = -(Im(x[8])+Im(x[5]));
	tmp49r = Re(x[8])-Re(x[5]);
	tmp50i = Im(x[3])-Im(x[10]);
	tmp50r = Re(x[3])+Re(x[10]);
	tmp51i = Im(x[3])+Im(x[10]);
	tmp51r = Re(x[3])-Re(x[10]);
	tmp52i = -(Im(x[6])-Im(x[7]));
	tmp52r = Re(x[6])+Re(x[7]);
	tmp53i = -(Im(x[6])+Im(x[7]));
	tmp53r = Re(x[6])-Re(x[7]);
	tmp54i = C15*tmp42i+C17*tmp44i-C19*tmp46i-C21*tmp48i+C23*tmp50i-C25*tmp52i-Im(x[0]);
	tmp54r = C15*tmp42r+C17*tmp44r-C19*tmp46r-C21*tmp48r+C23*tmp50r-C25*tmp52r+Re(x[0]);
	tmp55r = tmp42r+tmp44r;
	tmp56i = C17*tmp42i-C19*tmp44i-C21*tmp46i+C23*tmp48i-C25*tmp50i+C15*tmp52i-Im(x[0]);
	tmp56r = C17*tmp42r-C19*tmp44r-C21*tmp46r+C23*tmp48r-C25*tmp50r+C15*tmp52r+Re(x[0]);
	tmp57r = tmp55r+tmp46r;
	tmp58i = -C19*tmp42i-C21*tmp44i+C23*tmp46i-C25*tmp48i+C15*tmp50i+C17*tmp52i-Im(x[0]);
	tmp58r = -C19*tmp42r-C21*tmp44r+C23*tmp46r-C25*tmp48r+C15*tmp50r+C17*tmp52r+Re(x[0]);
	tmp59r = tmp57r+tmp48r;
	tmp60i = -C21*tmp42i+C23*tmp44i-C25*tmp46i+C15*tmp48i+C17*tmp50i-C19*tmp52i-Im(x[0]);
	tmp60r = -C21*tmp42r+C23*tmp44r-C25*tmp46r+C15*tmp48r+C17*tmp50r-C19*tmp52r+Re(x[0]);
	tmp61r = tmp59r+tmp50r;
	tmp62i = C23*tmp42i-C25*tmp44i+C15*tmp46i+C17*tmp48i-C19*tmp50i-C21*tmp52i-Im(x[0]);
	tmp62r = C23*tmp42r-C25*tmp44r+C15*tmp46r+C17*tmp48r-C19*tmp50r-C21*tmp52r+Re(x[0]);
	tmp63r = tmp61r+tmp52r;
	tmp64i = -C25*tmp42i+C15*tmp44i+C17*tmp46i-C19*tmp48i-C21*tmp50i+C23*tmp52i-Im(x[0]);
	tmp64r = -C25*tmp42r+C15*tmp44r+C17*tmp46r-C19*tmp48r-C21*tmp50r+C23*tmp52r+Re(x[0]);
...
void	MFFTR28(Complex *x)
{
	...
	tmp16r = tmp7r-tmp10r;
	tmp17r = Re(x[0])+tmp6r;
	tmp3i = Im(x[1])+Im(x[13]);
	tmp18r = Re(x[1])+Re(x[13]);
	tmp4i = Im(x[1])-Im(x[13]);
	tmp19r = Re(x[1])-Re(x[13]);
	tmp5i = Im(x[11])+Im(x[3]);
	tmp20r = -(Re(x[11])+Re(x[3]));
	tmp6i = Im(x[11])-Im(x[3]);
	tmp21r = -(Re(x[11])-Re(x[3]));
	tmp7i = Im(x[9])+Im(x[5]);
	tmp22r = Re(x[9])+Re(x[5]);
	tmp8i = Im(x[9])-Im(x[5]);
	tmp23r = Re(x[9])-Re(x[5]);
	tmp9i = C9*tmp3i-C11*tmp5i-C13*tmp7i+Im(x[7]);
	tmp24r = C9*tmp18r-C11*tmp20r-C13*tmp22r-Re(x[7]);
	tmp10i = tmp3i+tmp5i;
	tmp25r = tmp18r+tmp20r;
	tmp11i = -C11*tmp3i-C13*tmp5i+C9*tmp7i+Im(x[7]);
	tmp26r = -C11*tmp18r-C13*tmp20r+C9*tmp22r-Re(x[7]);
	tmp12i = tmp10i+tmp7i;
	tmp27r = tmp25r+tmp22r;
	tmp13i = -C13*tmp3i+C9*tmp5i-C11*tmp7i+Im(x[7]);
	tmp28r = -C13*tmp18r+C9*tmp20r-C11*tmp22r-Re(x[7]);
	...
	tmp23i = Im(x[7])+tmp12i;
	tmp38r = -(Re(x[7])-tmp27r);
	tmp39r = C16*Re(x[6]);
	tmp24i = -C16*Im(x[6]);
	tmp40r = C16*Re(x[10]);
	tmp25i = C16*Im(x[10]);
	...
	tmp27i = Im(x[13])+Im(x[1]);
	tmp56r = -(Re(x[13])+Re(x[1]));
	tmp28i = Im(x[13])-Im(x[1]);
	tmp57r = -(Re(x[13])-Re(x[1]));
	tmp29i = Im(x[3])+Im(x[11]);
	tmp58r = Re(x[3])+Re(x[11]);
	tmp30i = Im(x[3])-Im(x[11]);
	tmp59r = Re(x[3])-Re(x[11]);
	tmp31i = Im(x[5])+Im(x[9]);
	tmp60r = -(Re(x[5])+Re(x[9]));
	tmp32i = Im(x[5])-Im(x[9]);
	tmp61r = -(Re(x[5])-Re(x[9]));
	tmp33i = C9*tmp27i-C11*tmp29i-C13*tmp31i+Im(x[7]);
	tmp62r = C9*tmp56r-C11*tmp58r-C13*tmp60r+Re(x[7]);
	tmp34i = tmp27i+tmp29i;
	tmp63r = tmp56r+tmp58r;
	tmp35i = -C11*tmp27i-C13*tmp29i+C9*tmp31i+Im(x[7]);
	tmp64r = -C11*tmp56r-C13*tmp58r+C9*tmp60r+Re(x[7]);
	tmp36i = tmp34i+tmp31i;
	tmp65r = tmp63r+tmp60r;
	tmp37i = -C13*tmp27i+C9*tmp29i-C11*tmp31i+Im(x[7]);
	tmp66r = -C13*tmp56r+C9*tmp58r-C11*tmp60r+Re(x[7]);
	...
	tmp47i = Im(x[7])+tmp36i;
	tmp76r = Re(x[7])+tmp65r;
...
void	MIFFTR28(Complex *x)
{
	...
	tmp1i = Im(x[8])+Im(x[6]);
	tmp1r = Re(x[8])-Re(x[6]);
	tmp2i = -(Im(x[10])-Im(x[4]));
	tmp2r = Re(x[10])+Re(x[4]);
	tmp3i = -(Im(x[10])+Im(x[4]));
	tmp3r = Re(x[10])-Re(x[4]);
	tmp4i = -(Im(x[2])-Im(x[12]));
	tmp4r = Re(x[2])+Re(x[12]);
	tmp5i = -(Im(x[2])+Im(x[12]));
	tmp5r = Re(x[2])-Re(x[12]);
	tmp6i = C9*tmp0i-C11*tmp2i-C13*tmp4i+Im(x[0]);
	tmp6r = C9*tmp0r-C11*tmp2r-C13*tmp4r+Re(x[0]);
	tmp7i = tmp0i+tmp2i;
	tmp7r = tmp0r+tmp2r;
	...
	tmp20i = Im(x[0])+tmp9i;
	tmp20r = Re(x[0])+tmp9r;
	tmp21i = Im(x[1])+Im(x[13]);
	tmp21r = Re(x[1])+Re(x[13]);
	tmp22i = Im(x[1])-Im(x[13]);
	tmp22r = Re(x[1])-Re(x[13]);
	tmp23i = -(Im(x[3])+Im(x[11]));
	tmp23r = Re(x[3])+Re(x[11]);
	tmp24i = -(Im(x[3])-Im(x[11]));
	tmp24r = Re(x[3])-Re(x[11]);
	tmp25i = Im(x[9])+Im(x[5]);
	tmp25r = Re(x[9])+Re(x[5]);
	tmp26i = Im(x[9])-Im(x[5]);
	tmp26r = Re(x[9])-Re(x[5]);
	tmp27i = C9*tmp21i-C11*tmp23i-C13*tmp25i-Im(x[7]);
	tmp27r = C9*tmp21r-C11*tmp23r-C13*tmp25r+Re(x[7]);
	tmp28i = tmp21i+tmp23i;
	tmp28r = tmp21r+tmp23r;
	tmp29i = -C11*tmp21i-C13*tmp23i+C9*tmp25i-Im(x[7]);
	tmp29r = -C11*tmp21r-C13*tmp23r+C9*tmp25r+Re(x[7]);
	tmp30i = tmp28i+tmp25i;
	tmp30r = tmp28r+tmp25r;
	tmp31i = -C13*tmp21i+C9*tmp23i-C11*tmp25i-Im(x[7]);
	tmp31r = -C13*tmp21r+C9*tmp23r-C11*tmp25r+Re(x[7]);
	tmp32i = C10*tmp22r+C12*tmp24r+C14*tmp26r;
	tmp32r = -C10*tmp22i-C12*tmp24i-C14*tmp26i;
	tmp33i = C12*tmp22r+C14*tmp24r-C10*tmp26r;
	tmp33r = -C12*tmp22i-C14*tmp24i+C10*tmp26i;
	...
	tmp40i = tmp31i-tmp34i;
	tmp40r = tmp31r-tmp34r;
	tmp41i = -(Im(x[7])-tmp30i);
	tmp41r = Re(x[7])+tmp30r;
	tmp42i = -(Im(x[8])-Im(x[6]));
	tmp42r = Re(x[8])+Re(x[6]);
	tmp43i = -(Im(x[8])+Im(x[6]));
	tmp43r = Re(x[8])-Re(x[6]);
	tmp44i = Im(x[10])-Im(x[4]);
	tmp44r = Re(x[10])+Re(x[4]);
	tmp45i = Im(x[10])+Im(x[4]);
	tmp45r = Re(x[10])-Re(x[4]);
	tmp46i = Im(x[2])-Im(x[12]);
	tmp46r = Re(x[2])+Re(x[12]);
	tmp47i = Im(x[2])+Im(x[12]);
	tmp47r = Re(x[2])-Re(x[12]);
	tmp48i = C9*tmp42i-C11*tmp44i-C13*tmp46i-Im(x[0]);
	tmp48r = C9*tmp42r-C11*tmp44r-C13*tmp46r+Re(x[0]);
	tmp49i = tmp42i+tmp44i;
	tmp49r = tmp42r+tmp44r;
	tmp50i = -C11*tmp42i-C13*tmp44i+C9*tmp46i-Im(x[0]);
	tmp50r = -C11*tmp42r-C13*tmp44r+C9*tmp46r+Re(x[0]);
	tmp51i = tmp49i+tmp46i;
	tmp51r = tmp49r+tmp46r;
	tmp52i = -C13*tmp42i+C9*tmp44i-C11*tmp46i-Im(x[0]);
	tmp52r = -C13*tmp42r+C9*tmp44r-C11*tmp46r+Re(x[0]);
	...
	tmp62i = -(Im(x[0])-tmp51i);
	tmp62r = Re(x[0])+tmp51r;
	tmp63i = -(Im(x[1])+Im(x[13]));
	tmp63r = Re(x[1])+Re(x[13]);
	tmp64i = -(Im(x[1])-Im(x[13]));
	tmp64r = Re(x[1])-Re(x[13]);
	tmp65i = Im(x[3])+Im(x[11]);
	tmp65r = Re(x[3])+Re(x[11]);
	tmp66i = Im(x[3])-Im(x[11]);
	tmp66r = Re(x[3])-Re(x[11]);
	tmp67i = -(Im(x[9])+Im(x[5]));
	tmp67r = Re(x[9])+Re(x[5]);
	tmp68i = -(Im(x[9])-Im(x[5]));
	tmp68r = Re(x[9])-Re(x[5]);
	tmp69i = C9*tmp63i-C11*tmp65i-C13*tmp67i+Im(x[7]);
	tmp69r = C9*tmp63r-C11*tmp65r-C13*tmp67r+Re(x[7]);
	tmp70i = tmp63i+tmp65i;
	tmp70r = tmp63r+tmp65r;
	tmp71i = -C11*tmp63i-C13*tmp65i+C9*tmp67i+Im(x[7]);
	tmp71r = -C11*tmp63r-C13*tmp65r+C9*tmp67r+Re(x[7]);
	tmp72i = tmp70i+tmp67i;
	tmp72r = tmp70r+tmp67r;
	tmp73i = -C13*tmp63i+C9*tmp65i-C11*tmp67i+Im(x[7]);
	tmp73r = -C13*tmp63r+C9*tmp65r-C11*tmp67r+Re(x[7]);
	...
	tmp83i = Im(x[7])+tmp72i;
	tmp83r = Re(x[7])+tmp72r;
...
void	MFFTR30(Complex *x)
{
	...
	const REAL	C1 =                 0.25;	/* REALCONST	*/
	const REAL	C13 =                  0.5;	/* REALCONST	*/
	const REAL	C15 =     0.55901699437495;	/* REALCONST	*/
	const REAL	C10 =     0.58778525229247;	/* REALCONST	*/
	const REAL	C17 =     0.86602540378444;	/* REALCONST	*/
	const REAL	C8 =     0.95105651629515;	/* REALCONST	*/
	const REAL	C12 =                    2;	/* INTEGERCONST	*/
	...
	tmp14r = tmp8r-tmp10r;
	tmp15r = Re(x[0])+tmp2r;
	tmp2i = Im(x[1])+Im(x[11]);
	tmp16r = Re(x[1])-Re(x[11]);
	tmp3i = Im(x[1])-Im(x[11]);
	tmp17r = Re(x[1])+Re(x[11]);
	tmp4i = Im(x[7])+Im(x[13]);
	tmp18r = Re(x[7])+Re(x[13]);
	tmp5i = Im(x[7])-Im(x[13]);
	tmp19r = Re(x[7])-Re(x[13]);
	tmp6i = tmp2i+tmp4i;
	tmp20r = tmp16r+tmp18r;
	tmp7i = tmp2i-tmp4i;
	tmp21r = tmp16r-tmp18r;
	...
	tmp19i = Im(x[5])+tmp6i;
	tmp33r = -(Re(x[5])-tmp20r);
	tmp20i = -(Im(x[4])-Im(x[14]));
	tmp34r = Re(x[4])+Re(x[14]);
	tmp21i = -(Im(x[4])+Im(x[14]));
	tmp35r = Re(x[4])-Re(x[14]);
	tmp22i = Im(x[2])+Im(x[8]);
	tmp36r = Re(x[2])+Re(x[8]);
	tmp23i = Im(x[2])-Im(x[8]);
	tmp37r = Re(x[2])-Re(x[8]);
	...
	tmp27i = C15*tmp25i;
	tmp41r = C15*tmp39r;
	tmp28i = tmp26i-Im(x[10]);
	tmp42r = tmp40r+Re(x[10]);
	...
	tmp37i = -(Im(x[10])-tmp24i);
	tmp51r = Re(x[10])+tmp38r;
	tmp38i = C12*Im(x[9]);
	tmp52r = -C12*Re(x[9]);
	tmp39i = C12*Im(x[3]);
	tmp53r = -C12*Re(x[3]);
	...
	tmp52i = -(Im(x[14])-Im(x[4]));
	tmp54r = Re(x[14])+Re(x[4]);
	tmp53i = -(Im(x[14])+Im(x[4]));
	tmp55r = Re(x[14])-Re(x[4]);
	tmp54i = -(Im(x[8])+Im(x[2]));
	tmp56r = Re(x[8])+Re(x[2]);
	tmp55i = -(Im(x[8])-Im(x[2]));
	tmp57r = Re(x[8])-Re(x[2]);
	...
	tmp59i = C15*tmp57i;
	tmp61r = C15*tmp59r;
	tmp60i = tmp58i+Im(x[10]);
	tmp62r = tmp60r+Re(x[10]);
	tmp61i = tmp60i+tmp59i;
	tmp63r = tmp62r+tmp61r;
	...
	tmp70r = tmp64r-tmp66r;
	tmp69i = Im(x[10])+tmp56i;
	tmp71r = Re(x[10])+tmp58r;
	tmp70i = Im(x[11])+Im(x[1]);
	tmp72r = Re(x[11])-Re(x[1]);
	tmp71i = Im(x[11])-Im(x[1]);
	tmp73r = Re(x[11])+Re(x[1]);
	tmp72i = Im(x[13])+Im(x[7]);
	tmp74r = -(Re(x[13])+Re(x[7]));
	tmp73i = Im(x[13])-Im(x[7]);
	tmp75r = -(Re(x[13])-Re(x[7]));
	...
	tmp77i = C15*tmp75i;
	tmp79r = C15*tmp77r;
	tmp78i = tmp76i+Im(x[5]);
	tmp80r = tmp78r+Re(x[5]);
	...
	tmp87i = Im(x[5])+tmp74i;
	tmp89r = Re(x[5])+tmp76r;
...
void	MIFFTR30(Complex *x)
{
	...
	tmp1i = Im(x[6])+Im(x[9]);
	tmp1r = Re(x[6])-Re(x[9]);
	tmp2i = Im(x[12])-Im(x[3]);
	tmp2r = Re(x[12])+Re(x[3]);
	tmp3i = Im(x[12])+Im(x[3]);
	tmp3r = Re(x[12])-Re(x[3]);
	tmp4i = tmp0i+tmp2i;
	tmp4r = tmp0r+tmp2r;
	tmp5i = tmp0i-tmp2i;
	tmp5r = tmp0r-tmp2r;
	...
	tmp17i = Im(x[0])+tmp4i;
	tmp17r = Re(x[0])+tmp4r;
	tmp18i = Im(x[1])-Im(x[4]);
	tmp18r = Re(x[1])+Re(x[4]);
	tmp19i = Im(x[1])+Im(x[4]);
	tmp19r = Re(x[1])-Re(x[4]);
	tmp20i = Im(x[7])+Im(x[13]);
	tmp20r = Re(x[7])+Re(x[13]);
	tmp21i = Im(x[7])-Im(x[13]);
	tmp21r = Re(x[7])-Re(x[13]);
	tmp22i = tmp18i+tmp20i;
	tmp22r = tmp18r+tmp20r;
	tmp23i = tmp18i-tmp20i;
	tmp23r = tmp18r-tmp20r;
	...
	tmp25i = C15*tmp23i;
	tmp25r = C15*tmp23r;
	tmp26i = tmp24i-Im(x[10]);
	tmp26r = tmp24r+Re(x[10]);
	tmp27i = tmp26i+tmp25i;
	tmp27r = tmp26r+tmp25r;
	tmp28i = tmp26i-tmp25i;
	tmp28r = tmp26r-tmp25r;
	...
	tmp34i = tmp28i-tmp30i;
	tmp34r = tmp28r-tmp30r;
	tmp35i = -(Im(x[10])-tmp22i);
	tmp35r = Re(x[10])+tmp22r;
	tmp36i = -(Im(x[11])-Im(x[14]));
	tmp36r = Re(x[11])+Re(x[14]);
	tmp37i = -(Im(x[11])+Im(x[14]));
	tmp37r = Re(x[11])-Re(x[14]);
	tmp38i = Im(x[2])+Im(x[8]);
	tmp38r = Re(x[2])+Re(x[8]);
	tmp39i = Im(x[2])-Im(x[8]);
	tmp39r = Re(x[2])-Re(x[8]);
	...
	tmp43i = C15*tmp41i;
	tmp43r = C15*tmp41r;
	tmp44i = tmp42i-Im(x[5]);
	tmp44r = tmp42r+Re(x[5]);
	tmp45i = tmp44i+tmp43i;
	tmp45r = tmp44r+tmp43r;
	tmp46i = tmp44i-tmp43i;
	tmp46r = tmp44r-tmp43r;
	...
	tmp52i = tmp46i-tmp48i;
	tmp52r = tmp46r-tmp48r;
	tmp53i = -(Im(x[5])-tmp40i);
	tmp53r = Re(x[5])+tmp40r;
	tmp54i = -(Im(x[6])-Im(x[9]));
	tmp54r = Re(x[6])+Re(x[9]);
	tmp55i = -(Im(x[6])+Im(x[9]));
	tmp55r = Re(x[6])-Re(x[9]);
	tmp56i = -(Im(x[12])-Im(x[3]));
	tmp56r = Re(x[12])+Re(x[3]);
	tmp57i = -(Im(x[12])+Im(x[3]));
	tmp57r = Re(x[12])-Re(x[3]);
	tmp58i = tmp54i+tmp56i;
	tmp58r = tmp54r+tmp56r;
	tmp59i = tmp54i-tmp56i;
	tmp59r = tmp54r-tmp56r;
	...
	tmp61i = C15*tmp59i;
	tmp61r = C15*tmp59r;
	tmp62i = tmp60i-Im(x[0]);
	tmp62r = tmp60r+Re(x[0]);
	tmp63i = tmp62i+tmp61i;
	tmp63r = tmp62r+tmp61r;
	tmp64i = tmp62i-tmp61i;
	tmp64r = tmp62r-tmp61r;
	...
	tmp70i = tmp64i-tmp66i;
	tmp70r = tmp64r-tmp66r;
	tmp71i = -(Im(x[0])-tmp58i);
	tmp71r = Re(x[0])+tmp58r;
	tmp72i = -(Im(x[1])-Im(x[4]));
	tmp72r = Re(x[1])+Re(x[4]);
	tmp73i = -(Im(x[1])+Im(x[4]));
	tmp73r = Re(x[1])-Re(x[4]);
	tmp74i = -(Im(x[7])+Im(x[13]));
	tmp74r = Re(x[7])+Re(x[13]);
	tmp75i = -(Im(x[7])-Im(x[13]));
	tmp75r = Re(x[7])-Re(x[13]);
	...
	tmp79i = C15*tmp77i;
	tmp79r = C15*tmp77r;
	tmp80i = tmp78i+Im(x[10]);
	tmp80r = tmp78r+Re(x[10]);
	...
	tmp88r = tmp82r-tmp84r;
	tmp89i = Im(x[10])+tmp76i;
	tmp89r = Re(x[10])+tmp76r;
	tmp90i = Im(x[11])-Im(x[14]);
	tmp90r = Re(x[11])+Re(x[14]);
	tmp91i = Im(x[11])+Im(x[14]);
	tmp91r = Re(x[11])-Re(x[14]);
	tmp92i = -(Im(x[2])+Im(x[8]));
	tmp92r = Re(x[2])+Re(x[8]);
	tmp93i = -(Im(x[2])-Im(x[8]));
	tmp93r = Re(x[2])-Re(x[8]);
	...
	tmp97i = C15*tmp95i;
	tmp97r = C15*tmp95r;
	tmp98i = tmp96i+Im(x[5]);
	tmp98r = tmp96r+Re(x[5]);
	tmp99i = tmp98i+tmp97i;
	...
	tmp107i = Im(x[5])+tmp94i;
	tmp107r = Re(x[5])+tmp94r;
...
void	MFFTR32(Complex *x)
{
	...
	tmp3r = Re(x[0])+tmp0i;
	tmp4r = Re(x[0])-tmp0i;
	tmp1i = Im(x[4])-Im(x[12]);
	tmp5r = Re(x[4])+Re(x[12]);
	tmp2i = Im(x[4])+Im(x[12]);
	tmp6r = Re(x[4])-Re(x[12]);
	tmp3i = Im(x[12])-Im(x[4]);
	tmp7r = Re(x[12])+Re(x[4]);
	tmp4i = Im(x[12])+Im(x[4]);
	tmp8r = Re(x[12])-Re(x[4]);
	tmp5i = tmp1i+tmp3i;
	tmp9r = tmp5r+tmp7r;
	tmp6i = tmp1i-tmp3i;
	tmp10r = tmp5r-tmp7r;
	...
	tmp21r = tmp4r+tmp14r;
	tmp22r = tmp4r-tmp14r;
	tmp11i = Im(x[2])-Im(x[14]);
	tmp23r = Re(x[2])+Re(x[14]);
	tmp12i = Im(x[2])+Im(x[14]);
	tmp24r = Re(x[2])-Re(x[14]);
	tmp13i = Im(x[10])-Im(x[6]);
	tmp25r = Re(x[10])+Re(x[6]);
	tmp14i = Im(x[10])+Im(x[6]);
	tmp26r = Re(x[10])-Re(x[6]);
	tmp15i = tmp11i+tmp13i;
	tmp27r = tmp23r+tmp25r;
	tmp16i = tmp11i-tmp13i;
	tmp28r = tmp23r-tmp25r;
	...
	tmp23i = Im(x[6])-Im(x[10]);
	tmp35r = Re(x[6])+Re(x[10]);
	tmp24i = Im(x[6])+Im(x[10]);
	tmp36r = Re(x[6])-Re(x[10]);
	tmp25i = Im(x[14])-Im(x[2]);
	tmp37r = Re(x[14])+Re(x[2]);
	tmp26i = Im(x[14])+Im(x[2]);
	tmp38r = Re(x[14])-Re(x[2]);
	tmp27i = tmp23i+tmp25i;
	tmp39r = tmp35r+tmp37r;
	tmp28i = tmp23i-tmp25i;
	tmp40r = tmp35r-tmp37r;
	...
	tmp59i = Im(x[1])+Im(x[15]);
	tmp71r = Re(x[1])-Re(x[15]);
	tmp60i = Im(x[1])-Im(x[15]);
	tmp72r = Re(x[1])+Re(x[15]);
	tmp61i = Im(x[9])+Im(x[7]);
	tmp73r = Re(x[9])-Re(x[7]);
	tmp62i = Im(x[9])-Im(x[7]);
	tmp74r = Re(x[9])+Re(x[7]);
	...
	tmp67i = Im(x[5])+Im(x[11]);
	tmp79r = Re(x[5])-Re(x[11]);
	tmp68i = Im(x[5])-Im(x[11]);
	tmp80r = Re(x[5])+Re(x[11]);
	tmp69i = Im(x[13])+Im(x[3]);
	tmp81r = Re(x[13])-Re(x[3]);
	tmp70i = Im(x[13])-Im(x[3]);
	tmp82r = Re(x[13])+Re(x[3]);
	...
	tmp93i = Im(x[3])+Im(x[13]);
	tmp105r = Re(x[3])-Re(x[13]);
	tmp94i = Im(x[3])-Im(x[13]);
	tmp106r = Re(x[3])+Re(x[13]);
	tmp95i = Im(x[11])+Im(x[5]);
	tmp107r = Re(x[11])-Re(x[5]);
	tmp96i = Im(x[11])-Im(x[5]);
	tmp108r = Re(x[11])+Re(x[5]);
	...
	tmp101i = Im(x[7])+Im(x[9]);
	tmp113r = Re(x[7])-Re(x[9]);
	tmp102i = Im(x[7])-Im(x[9]);
	tmp114r = Re(x[7])+Re(x[9]);
	tmp103i = Im(x[15])+Im(x[1]);
	tmp115r = Re(x[15])-Re(x[1]);
	tmp104i = Im(x[15])-Im(x[1]);
	tmp116r = Re(x[15])+Re(x[1]);
