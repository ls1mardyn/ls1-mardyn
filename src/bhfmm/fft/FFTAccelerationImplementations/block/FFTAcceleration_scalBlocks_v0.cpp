...
FFTDataContainer_scalBlocks_v0* FFTAcceleration_scalBlocks_v0::getFFTData(FFTAccelerableExpansion & Expansion)
{
  FFTDataContainer_scalBlocks_v0* FFTData = NULL;
  if(! Expansion.issetFFTData()) {
    FFTData = new FFTDataContainer_scalBlocks_v0(_nbBlocks, _fft_nx, _fft_ny, false, _blockSize);
  ...
  {
    FFTData = static_cast<FFTDataContainer_scalBlocks_v0*>(Expansion._FFTData);
  }
  
  return FFTData;
}

FFTDataContainer_scalBlocks_v0* FFTAcceleration_scalBlocks_v0::getFFTData_scal(FFTAccelerableExpansion & Expansion)
{
  FFTDataContainer_scalBlocks_v0* FFTData = NULL;
  if(! Expansion.issetFFTData()) {
    FFTData = new FFTDataContainer_scalBlocks_v0(_nbBlocks, _fft_nx, _fft_ny, true, _blockSize);
...
void FFTAcceleration_scalBlocks_v0::FFT_initialize_Source(FFTAccelerableExpansion & Expansion, double radius)
{
  FFTDataContainer_scalBlocks_v0* FFTData = getFFTData(Expansion);
...
void FFTAcceleration_scalBlocks_v0::FFT_finalize_Target(FFTAccelerableExpansion & Expansion, double radius)
{
  FFTDataContainer_scalBlocks_v0* FFTData = getFFTData(Expansion);
  FFT_precision*** & Re = FFTData->Re;
  FFT_precision*** & Im = FFTData->Im;  
...
void FFTAcceleration_scalBlocks_v0::FFT_M2L(FFTAccelerableExpansion & Source, FFTAccelerableExpansion & Target, FFTDataContainer* TransferFunction)
{
  ...
  FFTDataContainer_scalBlocks_v0* Tf_FFTData = static_cast<FFTDataContainer_scalBlocks_v0*>(TransferFunction);
    ...
    for(b=0; b<=lastBlock-currentBlock; b++) {
      ...
      if(tfBlock < _nbBlocks/2) { //scaling blocks scheme
        ...
        for(j=0; j<_blockSize[tfBlock];j++) {
          j_rep = repetition * j;
          for(i=0; i<_fft_nx; i++) {
            T_Re[currentBlock][i][j_rep] += S_Re[b][i][j_rep] * Tf_Re[tfBlock][i][j] - S_Im[b][i][j_rep] * Tf_Im[tfBlock][i][j];
            T_Im[currentBlock][i][j_rep] += S_Re[b][i][j_rep] * Tf_Im[tfBlock][i][j] + S_Im[b][i][j_rep] * Tf_Re[tfBlock][i][j];
          }
        }
...
void FFTAcceleration_scalBlocks_v0::FFT_M2L_vec(FFTAccelerableExpansion & Source, FFTAccelerableExpansion & Target, FFTDataContainer* TransferFunction)
{return FFT_M2L(Source, Target, TransferFunction);} //TODO
