...
void FFTAcceleration_blocks_fftw::FFT_initialize_Source(FFTAccelerableExpansion & Expansion, double radius) 
{
  ...
  for(b=0; b<_nbBlocks; b++)
  {
    ...
    for(i=0; i<_nbLinePerBlock; i++) {
      ...
      for(j=0; j<_fft_ny; j++) {
        ...
        {
          if(j-_p+1 < 0) {
            minus_one_neg_j = 1.0 - 2.0*(FFT_precision)((_p-1-j)%2);
            in[i*_fft_ny+j][0] = (FFT_precision)Expansion.get_C(trueI,_p-1-j) * minus_one_neg_j;
            in[i*_fft_ny+j][1] = (FFT_precision)Expansion.get_S(trueI,_p-1-j) * -minus_one_neg_j;
          } else {
            in[i*_fft_ny+j][0] = (FFT_precision)Expansion.get_C(trueI,j-_p+1);
            in[i*_fft_ny+j][1] = (FFT_precision)Expansion.get_S(trueI,j-_p+1);
          }
          
          in[i*_fft_ny+j][0] = minus_one_power_i  * in[i*_fft_ny+j][0] / r;
          in[i*_fft_ny+j][1] = -minus_one_power_i * in[i*_fft_ny+j][1] / r; //we want to use use conj(Source) 
  ...
  for(b=0; b<_nbBlocks; b++)
  {
    ...
    for(i=0; i<_nbLinePerBlock; i++) {
      ...
      for(j=0; j<_fft_ny; j++) {
        ...
        {
          if(_p-1-j < 0) {
            minus_one_neg_j = 1.0 - 2.0*(FFT_precision)((j+1-_p)%2);
            in[(_nbLinePerBlock-i-1)*_fft_ny+j][0] = (FFT_precision)Expansion.get_C(trueI,j+1-_p) * minus_one_neg_j;
            in[(_nbLinePerBlock-i-1)*_fft_ny+j][1] = (FFT_precision)Expansion.get_S(trueI,j+1-_p) * -minus_one_neg_j;
          } else {
            in[(_nbLinePerBlock-i-1)*_fft_ny+j][0] = (FFT_precision)Expansion.get_C(trueI,_p-1-j);
            in[(_nbLinePerBlock-i-1)*_fft_ny+j][1] = (FFT_precision)Expansion.get_S(trueI,_p-1-j);
          }
...
void FFTAcceleration_blocks_fftw::FFT_finalize_Target(FFTAccelerableExpansion & Expansion, double radius)
{
  FFTDataContainer_blocks* FFTData = getFFTData(Expansion);
  FFT_precision*** & Re = FFTData->Re;
  FFT_precision*** & Im = FFTData->Im;  
  ...
  for(b=0;b<_nbBlocks;b++) {
    ...
    for (i=0; i<_nbLinePerBlock; i++) 
    {
      ...
      for (j=0; j<=trueI; j++) 
      {
        Expansion.get_C(trueI,j) += (double)(out[(_nbLinePerBlock-i-1)*_fft_ny+(_fft_ny-j-1)][0] * scaling);
        Expansion.get_S(trueI,j) += (double)(out[(_nbLinePerBlock-i-1)*_fft_ny+(_fft_ny-j-1)][1] * scaling);
      }
    ...
    if(b!=0)
      for (i=1; i<_nbLinePerBlock; i++) 
      {
        ...
        for (j=0; j<=trueI; j++) 
        {
          Expansion.get_C(trueI,j) += (double)(out[(_nbLinePerBlock*2-i-1)*_fft_ny+(_fft_ny-j-1)][0] * scaling2);
          Expansion.get_S(trueI,j) += (double)(out[(_nbLinePerBlock*2-i-1)*_fft_ny+(_fft_ny-j-1)][1] * scaling2);
        }
