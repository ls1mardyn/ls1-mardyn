...
void FFTAcceleration_2wayM2L_optFFT::FFT_initialize_Source(FFTAccelerableExpansion & Expansion, double radius) 
{
...
void FFTAcceleration_2wayM2L_optFFT::FFT_finalize_Target(FFTAccelerableExpansion & Expansion, double radius)
{
  FFTDataContainer_arrays* FFTData = getFFTData(Expansion);
  FFT_precision* & Re_a = FFTData->Re;
  FFT_precision* & Im_a = FFTData->Im;
...
void FFTAcceleration_2wayM2L_optFFT::matrix2array(FFT_precision** & m, FFT_precision* & a)
{
  int i,j,k;
  k = 0;
...
void FFTAcceleration_2wayM2L_optFFT::array2matrix(FFT_precision* & a, FFT_precision** & m)
{
  int i,j,k;
  k = 0;
  for(i=0;i<_fft_nx/2;i++)
