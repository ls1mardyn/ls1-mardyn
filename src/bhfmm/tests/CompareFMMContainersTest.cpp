...
void CompareFMMContainersTest::compare(double cutoffRadius, bool periodic) {
	...
	for (ParticleIterator m = LCUniform->iterator(); m.isValid(); m.next()) {
		m->calcFM();
	}
	...
	for (ParticleIterator m = LCAdaptive->iterator(); m.isValid(); m.next()) {
		m->calcFM();
	}
	...
	for(; itUniform.isValid() and itAdaptive.isValid(); itUniform.next(), itAdaptive.next()) {
