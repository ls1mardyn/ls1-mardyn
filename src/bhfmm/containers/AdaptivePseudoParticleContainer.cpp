...
const int split_target = 0, split_source = 1, split_internal = 3,
		NO_WORK = 5, M2L = 6, P2P = 7, PUSH = 8, POP = 9;
...
void AdaptivePseudoParticleContainer::build(ParticleContainer* pc) {
	...
	double ctr[3] = {_domainLength[0]/2, _domainLength[1]/2, _domainLength[2]/2};
	if(_threshold == 0){
		int depth = log2((_domainLength[0] / _cellLength[0]) * _subdivisionFactor);
		root = new dtt::DttNode(_Cells, _threshold, ctr, _domainLength,
			_maxOrd, depth);	
	}else{
		root = new dtt::DttNode(_Cells, _threshold, ctr, _domainLength, 
			_maxOrd,0);	
...
void AdaptivePseudoParticleContainer::horizontalPass(VectorizedChargeP2PCellProcessor* cp) {
	// M2l and P2P
	vc_p2p_cp = cp;
	while(stack.size() > 0){
		work_on_stack();
...
void AdaptivePseudoParticleContainer::buildHaloTrees(){
	...
	double ctr[3] = {_domainLength[0]/2, _domainLength[1]/2, _domainLength[2]/2};
 	...
 	bhfmm::Vector3<double> shift;
        halo_node = new dtt::DttNode(_Cells,_threshold,ctr,_domainLength,_maxOrd,root_depth,true);
        TargetSourceTupel tst;
	tst.target = root;
...
void AdaptivePseudoParticleContainer::push_on_stack(dtt::DttNode * trg, dtt::DttNode * src,
        Vector3<double> shift,int split = split_target){
...
void AdaptivePseudoParticleContainer::work_on_stack(){
	...
	if(dist > epsilon || dist < -epsilon){
		if(((tst.target->_domLen[0] + tst.source->_domLen[0]) / dist < 1 + epsilon)
		                && comp_r <= epsilon && comp_r >= -epsilon){
			print_stack_op(M2L);
			tst.target->m2l(tst.source->_mpCell.multipole,tst.shift);
			if(tst.shift[0] == 0.0 && tst.shift[1] == 0.0 && tst.shift[2] == 0.0){
