	...
	while (inputfilestream && !inputfilestream.eof()) {
		...
		**/
		...
		else if(token == "Trho" || token == "tRho")
		{
			...
			_regionSampling->GetSampleRegion(nID)->SetParamProfiles
					( initSamplingProfiles, writeFrequencyProfiles, nNumShellsProfiles);
		}
		else if(token == "vdf" || token == "VDF")
		{
			...
			_regionSampling->GetSampleRegion(nID)->SetParamVDF
					( initSamplingVDF, writeFrequencyVDF, nNumShellsVDF, nNumDiscreteStepsVDF, dVeloMax);
		}
		else
		{
