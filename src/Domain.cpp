   ...
   ParticleContainer* molCont, double dtConstantAcc )
{
   ...
   //
   domainDecomp->collCommInit(4*this->_localN.size());
        ...
        lNit++ )
   {
	 ...
	 domainDecomp->collCommAppendDouble(
	    this->_localVelocitySum[d][lNit->first]
	 );
   }
   domainDecomp->collCommAllreduceSum();
   for( map<unsigned, unsigned long>::iterator lNit = _localN.begin();
...
void Domain::setTargetT(int thermostat, double targetT)
{
   if(thermostat < 0)
   {
      if(!this->_localRank)
      {
         cout << "Warning: thermostat \'" << thermostat << "\' (T = "
	      << targetT << ") will be ignored.\n";
      }
      return;
   }
 ...
 * archaic version
 *
void Domain::writeCheckpoint(
   string filename, ParticleContainer* particleContainer,
   DomainDecompBase* domainDecomp
) {
  ofstream checkpointfilestream(filename.c_str());
  checkpointfilestream << "mardyn\t20090722"<< endl;
