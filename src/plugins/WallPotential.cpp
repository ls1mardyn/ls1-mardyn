...
void WallPotential::readXML(XMLfileUnits &xmlconfig) {
    ...
    if(potential == 93){
        ...
        global_log -> info() << "[WallPotential] No Potential was specified -> Default: LJ9_3" << std::endl;
        _potential = LJ9_3;
    }
    ...
    if(_potential == LJ9_3){
        ...
        global_log->info() << "Wallfun LJ9_3 initialized." << endl;
    }
    else if(_potential == LJ10_4){
        this->initializeLJ104(components, density, sigma, epsilon, xi_sf, eta_sf, yoff, ycut);
        global_log->info() << "Wallfun LJ10_4 initialized." << endl;
    }
    else{
        global_log -> error() << "UNKNOWN WALL POTENTIAL! EXITING!" << std::endl;
...
void WallPotential::afterForces(ParticleContainer *particleContainer, DomainDecompBase *domainDecomp,
    ...
    if(_potential == LJ9_3){
        ...
        global_log->debug() << "Wallfun LJ9_3 applied." << endl;
    }
    else if(_potential == LJ10_4){
        this->calcTSLJ_10_4(particleContainer);
        global_log->debug() << "Wallfun LJ10_4 applied." << endl;
    }
