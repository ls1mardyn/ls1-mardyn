...
void COMalignerBubble::readXML(XMLfileUnits& xmlconfig){
    ...
    if(_interval < 1 || _alignmentCorrection < 0 || _alignmentCorrection > 1){
        Log::global_log -> error() << "[COMalignerB] INVALID CONFIGURATION!!! DISABLED!" << std::endl;
        Log::global_log -> error() << "[COMalignerB] HALTING SIMULATION" << std::endl;
        _enabled = false;
        // HALT SIM
        Simulation::exit(1);
        return;
    ...
    Log::global_log -> info() << "[COMalignerB] settings:" << std::endl;
    ...
    Log::global_log -> debug() << "[COMalignerB] dim settings are: " << _dim_start << " " << _dim_end << " " << _dim_step << std::endl;
...
void COMalignerBubble::beforeForces(ParticleContainer* particleContainer,
    ...
    if(_enabled) {
        ...
        Log::global_log->debug() << "[COMalignerB] before forces called" << std::endl;
        ...
        Log::global_log->info() << "[COMalignerB] motion is x: " << _motion[0] << " y: " << _motion[1] << " z: " << _motion[2]
                           << std::endl;
        ...
        if(totalMotion > _cutoff){
            ...
            Log::global_log->info() << "[COMalignerB] Motion larger than Cutoff Radius. Reducing Motion" << _motion[2]
                               << std::endl;
            Log::global_log->info() << "[COMalignerB] New motion is x: " << _motion[0] << " y: " << _motion[1] << " z: " << _motion[2]
                               << std::endl;
        }
        ...
        Log::global_log->info() << "[COMalignerB] DISABLED, all dims set to 0" << std::endl;
    }
