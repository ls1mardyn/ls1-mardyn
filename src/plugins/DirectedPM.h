 ...
 * Temperature is then calculated by subtracting the overall droplet velocity from the individual particle
 * velocities<br> to yield only the thermal velocities. Alternatively, temperature is also calculated by only
 * considering velocities in x-<br> and z-direction. The droplet is assumed to move in y-direction.<br>
 * \code{.xml}
 * <plugin>
...
public:
	void init(ParticleContainer* particleContainer, DomainDecompBase* domainDecomp, Domain* domain) override {
		...
		// WRITE ALL SIMSTEPS, gerichtete Geschwindigkeit, dichteGas, dichteLiq, druckGas, druckLiq, TGas, TLiq, EkinGas
		// und EkinLiq IN AN OUTPUT FILE
