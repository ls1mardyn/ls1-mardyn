...
void DirectedPM::readXML(XMLfileUnits& xmlconfig){
    ...
    xmlconfig.getNodeValue("rohCutLiq", _rohCutLiq);	// Minimum density of liquid phase, for higher densities the bins are declared as liquid phase
	xmlconfig.getNodeValue("maxDeviation", _percent);				// deviation of liquid and vapor phase density which is allowed for the calculation of the mean phase density given as a percentage
...
void DirectedPM::beforeForces(ParticleContainer* particleContainer, DomainDecompBase* domainDecomp, unsigned long simstep) {
       ...
       if(simstep>0) {
            ...
            for (auto temporaryMolecule = particleContainer->iterator(ParticleIterator::ONLY_INNER_AND_BOUNDARY); temporaryMolecule.isValid(); ++temporaryMolecule) {
                ...
                if (COMPONENT == (_component - 1)) {
                    ...
                    if (rUN >= _rIncrements) {}
                        ...
                        if ((rUN >= 0) && (hUN >= 0) && (phiUN >= 0) && (rUN < _rIncrements) && (hUN < _hIncrements) && (phiUN < _phiIncrements)) {
                            unID = (hUN * _rIncrements * _phiIncrements) + (rUN * _phiIncrements) + phiUN;
                        }
                        else {
                            global_log->error() << "INV PROFILE UNITS " << _universalInvProfileUnit[0] << " " << _universalInvProfileUnit[1] << " " << _universalInvProfileUnit[2] << "\n";
                            global_log->error() << "PROFILE UNITS " << _rIncrements << " " << _hIncrements << " " << _phiIncrements << "\n";
                        ...
                        _localDirYVelocity2[unID] += (temporaryMolecule->v(1) - _directedVelocityOld) * (temporaryMolecule->v(1) - _directedVelocityOld); //velocity2 without directed velocity of Droplet
            ...
            if (synsimstep == _outputFrequency) {
                ...
                for (unsigned unID = 1; unID <= (_rIncrements * _hIncrements * _phiIncrements); ++unID) {
                    ...
                    _temperatureBox[unID] = 2. / (3. * _globalnumberOfParticles[unID]) * 0.5 * (_globalXyzVelocities2[unID][0] + _globalXyzVelocities2[unID][1] + _globalXyzVelocities2[unID][2]);
                    _temperatureBoxXZ[unID] = 2. / (2. * _globalnumberOfParticles[unID]) * 0.5 * (_globalXyzVelocities2[unID][0]  + _globalXyzVelocities2[unID][2]);
                    _EkinBox[unID] = 0.5 * (_globalXyzVelocities2[unID][0] + _globalXyzVelocities2[unID][1] + _globalXyzVelocities2[unID][2]);
                    _virialBox[unID] = _temperatureBox[unID] * (_globalnumberOfParticles[unID] / _outputFrequency) / _volumebox + (1. / _volumebox) * (_globalXyzVi[unID][0] + _globalXyzVi[unID][1] + _globalXyzVi[unID][2]) / (3 * _outputFrequency);
                    //pressurexDROPLET = TxyzDROPLET * (particleDropletTrue/ _outputFrequency) / (_volumebox * sumBoxesLiquid) + 1. / (_volumebox * sumBoxesLiquid) * (viDroplet[0] / _outputFrequency);
                    // COUNT ALL VALUES IF BOX IS IN THE DROPLET
                    if ((_densityBox[unID] > rohCutLiqActual) && (_permissibleRange[unID] == 1)) {
                        _temperatureBox[unID] = 2. / (3. * _globalnumberOfParticles[unID]) * 0.5 * (_globalXyzVelocities2[unID][0] + _globalDirYVelocity2[unID] + _globalXyzVelocities2[unID][2]);
                        _EkinBox[unID] = 0.5 * (_globalXyzVelocities2[unID][0] + _globalDirYVelocity2[unID] + _globalXyzVelocities2[unID][2]);
                        _virialBox[unID] = _temperatureBox[unID] * (_globalnumberOfParticles[unID] / _outputFrequency) / _volumebox + (1. / _volumebox) * (_globalXyzVi[unID][0] + _globalXyzVi[unID][1] + _globalXyzVi[unID][2]) / (3 * _outputFrequency);
                        ...
                        _xyzEkinDroplet[_outputFrequency] += 0.5 * (_globalXyzVelocities2[unID][0] + _globalDirYVelocity2[unID] + _globalXyzVelocities2[unID][2]);
                        _xzEkinDroplet[_outputFrequency] += 0.5 * (_globalXyzVelocities2[unID][0] + _globalXyzVelocities2[unID][2]);
                        sumRohLiq += _densityBox[unID];
                        sumBoxesLiquid += 1.;
                        if (unID < UnIDMin) { UnIDMin = unID; }
                        if (unID > UnIDMax) { UnIDMax = unID; }
                    }
                }
                ...
                for (unsigned unID = 1; unID <= (_rIncrements * _hIncrements * _phiIncrements); ++unID) {
                     if ((_permissibleRange[unID] == 1)){
                        ...
                        _xyzEkinGas[_outputFrequency] += 0.5 * (_globalXyzVelocities2[unID][0] + _globalXyzVelocities2[unID][1] + _globalXyzVelocities2[unID][2]);
                        _xzEkinGas[_outputFrequency] += 0.5 * (_globalXyzVelocities2[unID][0] + _globalXyzVelocities2[unID][2]);
                        sumBoxesVapor += 1.;
                     }  
                ...
                _rohCutLiqNew = (particleDropletTrue/ _outputFrequency)/(sumBoxesLiquid*_volumebox);
                    ...
                    // CALCULATE GLOBAL TEMPERATURE --> Hierfür Ekin über alle Bins aufsummieren und durch particelnumberdroplet teilen
                    ...
                    // CALCULATE PRESSURE
                    double pressurexDROPLET = TxyzDROPLET * (particleDropletTrue / _outputFrequency) / (_volumebox * sumBoxesLiquid) + 1. / (_volumebox * sumBoxesLiquid) * (viDroplet[0] / _outputFrequency);
                    double pressureyDROPLET = TxyzDROPLET * (particleDropletTrue/ _outputFrequency) / (_volumebox * sumBoxesLiquid) + 1. / (_volumebox * sumBoxesLiquid) * (viDroplet[1] / _outputFrequency);
                    double pressurezDROPLET = TxyzDROPLET * (particleDropletTrue/ _outputFrequency) / (_volumebox * sumBoxesLiquid) + 1. / (_volumebox * sumBoxesLiquid) * (viDroplet[2] / _outputFrequency);
                    double pressurexGAS = TxyzGAS * (particleDropletFalse / _outputFrequency) / (_volumebox * sumBoxesVapor) + 1. / (_volumebox * sumBoxesVapor) * (viGas[0] / _outputFrequency);
                    double pressureyGAS = TxyzGAS * (particleDropletFalse / _outputFrequency) / (_volumebox * sumBoxesVapor) + 1. / (_volumebox * sumBoxesVapor) * (viGas[1] / _outputFrequency);
                    double pressurezGAS = TxyzGAS * (particleDropletFalse / _outputFrequency) / (_volumebox * sumBoxesVapor) + 1. / (_volumebox * sumBoxesVapor) * (viGas[2] / _outputFrequency); 
                    ...
                    if (mpi_rank == 0) {
                        ...
                        if (simstep / _outputFrequency > 1) {
                            _DPMGlobalStream << simstep << " \t\t" << _directedVelocityOld << " \t\t "
                                << ((particleDropletFalse / _outputFrequency) / (_volumebox * sumBoxesVapor)) << " \t\t " << _rohCutLiqNew << " \t\t " 
                                << ((pressurexGAS + pressureyGAS + pressurezGAS) / 3) << " \t\t " << ((pressurexDROPLET + pressureyDROPLET + pressurezDROPLET) / 3) << " \t\t " 
                                << TxyzGAS << " \t\t " << TxyzDROPLET << " \t\t " 
                                << _xyzEkinGas[_outputFrequency]/ (3*particleDropletFalse) << " \t\t " << _xyzEkinDroplet[_outputFrequency] / (3 * particleDropletTrue) << " \t\t "
                                << TxzGAS << " \t\t " << TxzDROPLET << " \t\t "
                                << _xzEkinGas[_outputFrequency]/ (2*particleDropletFalse) << " \t\t " << _xzEkinDroplet[_outputFrequency] / (2 * particleDropletTrue) << endl;
                        }
                        ...
                        DPMStreamDensity << "\t" << 1 / _universalInvProfileUnit[0] << "\t" << 1 / _universalInvProfileUnit[1] << "\t" << 1 / _universalInvProfileUnit[2] << "\n";
                        DPMStreamDensity << "0 \t";
                        for (unsigned r = 0; r < _rIncrements; r++) {
                            DPMStreamDensity << 0.5 * (sqrt(r + 1) + sqrt(r)) / sqrt(_universalInvProfileUnit[0]) << " \t"; // Eintragen der R-Koordinate in Header korrigiert 
                        }
                        ...
                        DPMStreamTemperature << "\t" << 1 / _universalInvProfileUnit[0] << "\t" << 1 / _universalInvProfileUnit[1] << "\t" << 1 / _universalInvProfileUnit[2] << "\n";
                        DPMStreamTemperature << "0 \t";
                        for (unsigned r = 0; r < _rIncrements; r++) {
                            DPMStreamTemperature << 0.5 * (sqrt(r + 1) + sqrt(r)) / sqrt(_universalInvProfileUnit[0]) << " \t"; // Eintragen der R-Koordinate in Header korrigiert
                        }
                        ...
                        DPMStreamTemperatureXZ << "//Temperature expressed by 2Ekin/#DOF\t MIT XZ-WERTEN!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
                        DPMStreamTemperatureXZ << "//local temperature profile: Y - Z || X-projection\n";
                        DPMStreamTemperatureXZ << "// \t dr \t dh \t dphi \n";
                        DPMStreamTemperatureXZ << "\t" << 1 / _universalInvProfileUnit[0] << "\t" << 1 / _universalInvProfileUnit[1] << "\t" << 1 / _universalInvProfileUnit[2] << "\n";
                        DPMStreamTemperatureXZ << "0 \t";
                        for (unsigned r = 0; r < _rIncrements; r++) {
                            DPMStreamTemperatureXZ << 0.5 * (sqrt(r + 1) + sqrt(r)) / sqrt(_universalInvProfileUnit[0]) << " \t"; // Eintragen der R-Koordinate in Header korrigiert
                        }
                        ...
                        DPMStreamEkin << "\t" << 1 / _universalInvProfileUnit[0] << "\t" << 1 / _universalInvProfileUnit[1] << "\t" << 1 / _universalInvProfileUnit[2] << "\n";
                        DPMStreamEkin << "0 \t";
                        for (unsigned r = 0; r < _rIncrements; r++) {
                            DPMStreamEkin << 0.5 * (sqrt(r + 1) + sqrt(r)) / sqrt(_universalInvProfileUnit[0]) << " \t"; // Eintragen der R-Koordinate in Header korrigiert
                        }
                        DPMStreamEkin << "\n";
                        // Y - axis label
                        for (unsigned h = 0; h < _hIncrements; h++) {
                        ...
                        DPMStreamVirial << "\t" << 1 / _universalInvProfileUnit[0] << "\t" << 1 / _universalInvProfileUnit[1] << "\t" << 1 / _universalInvProfileUnit[2] << "\n";
                        DPMStreamVirial << "0 \t";
                        for (unsigned r = 0; r < _rIncrements; r++) {
                            DPMStreamVirial << 0.5 * (sqrt(r + 1) + sqrt(r)) / sqrt(_universalInvProfileUnit[0]) << " \t"; // Eintragen der R-Koordinate in Header korrigiert
                        }
                        DPMStreamVirial << "\n";
                        // Y - axis label
                        for (unsigned h = 0; h < _hIncrements; h++) {
...
void DirectedPM::endStep(ParticleContainer* particleContainer, DomainDecompBase* domainDecomp, Domain* domain, unsigned long simstep) {
    if (_enabled) {
        if (simstep > 0) {
            // CALCULATE SYNSIMSTEP FOR MATRIX
            double synsimstep;
            ...
            // synsimstep als globale Variable deklarieren, dann ist Berechnung oben drüber unnötig
            // WRITE AN OUTPUT FILE TO CHECK THE DENSITYS AND THE POSITION OF THE DROP IN THE MIDDLE X-LAYER (ONLY FOR TESTS)
            if (synsimstep == _outputFrequency) {
