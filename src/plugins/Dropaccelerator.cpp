...
void Dropaccelerator::afterForces(ParticleContainer* particleContainer, DomainDecompBase* domainDecomp,
	...
	if (_enabled) {
		...
		if (simstep == _startSimStep) {
			global_log->info() << "afterForcesWiseForces Joshua !!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
		...
		if (simstep >= (_startSimStep + _steps) && simstep < (_startSimStep + 2 * _steps)) {
			...
			global_log->info() << "Geschwindigkeit jetzt = " << _velocNow << endl;
			global_log->info() << "Geschwindigkeits Differenz = " << deltavelocity << endl;
			global_log->info() << "corrVeloc = " << corrVeloc << endl;
			...
			if ((abs(_velocNow) < abs(_veloc))&&(abs(deltavelocity)>abs(corrVeloc))) {
				...
				global_log->info() << "ACCELERATION with corrVeloc" << endl;
				...
				global_log->info() << "NO ACCELERATION" << endl;
			}
		}
	}
}
