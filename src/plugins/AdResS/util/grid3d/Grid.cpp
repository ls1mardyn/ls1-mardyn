...
void Grid3D::Grid::StartGrid(int x, int y, int z){
...
namespace Grid3D {
	std::ostream& operator <<(std::ostream& out, Grid3D::Grid& grid){
		...
		for(int i=0;i<total_elements;i++){
			std::array<int, 8> nodes = grid.GetElementGlobalNodeIndices(i);
			out<<i<<"\t";
			for(int j=0;j<8;j++){
				out<<nodes[j]<<"\t";
			}
...
void Grid3D::Grid::AddSubset(const std::vector<int>& list, const std::string& Name){
    this->subsets.emplace_back(Name, list);
}

Subset& Grid3D::Grid::GetSubset(std::string& Name){
    auto found_subset = std::find_if(subsets.begin(),subsets.end(),
    [Name](Subset& ss){
        return ss.GetName()==Name;
    });
	if (found_subset != subsets.end()) return *found_subset;
	else throw std::runtime_error("name does not exist");
}

std::array<double, 3>& Grid3D::Grid::GetUpperCorner(){
    return upper_corner;
}

std::array<double, 3>& Grid3D::Grid::GetLowerCorner(){
    return lower_corner;
}

std::vector<Subset>& Grid3D::Grid::GetSubsets(){
    return this->subsets;
}