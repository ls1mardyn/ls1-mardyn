...
AdResSKDDecomposition::AdResSKDDecomposition(double cutoffRadius, int numParticleTypes, int updateFrequency,
...
void AdResSKDDecomposition::init(Domain* domain){
    int lowCorner[KDDIM]{};
    ...
    _numMoleculesPerCell.resize(static_cast<size_t>(_numParticleTypes) * _globalNumCells);
    ...
    if (!_decompTree->isResolvable()) {
        ...
        global_log->error() << "AdResSKDDecomposition not possible. Each process needs at least " << minCellCountPerProc
                            << " cells." << endl;
    ...
    global_log->info() << "Created AdResSKDDecomposition with updateFrequency=" << _frequency << ", fullSearchThreshold=" << _fullSearchThreshold << endl;
...
AdResSKDDecomposition::~AdResSKDDecomposition() {
...
void AdResSKDDecomposition::readXML(XMLfileUnits& xmlconfig) {
    ...
    global_log->info() << "AdResSKDDecomposition minNumCellsPerDimension: " << KDDStaticValues::minNumCellsPerDimension
					   << endl;
	if(KDDStaticValues::minNumCellsPerDimension==0u){
		global_log->error() << "AdResSKDDecomposition minNumCellsPerDimension has to be bigger than zero!" << std::endl;
		Simulation::exit(43);
	}
	xmlconfig.getNodeValue("updateFrequency", _frequency);
	global_log->info() << "AdResSKDDecomposition update frequency: " << _frequency << endl;
	xmlconfig.getNodeValue("fullSearchThreshold", _fullSearchThreshold);
	global_log->info() << "AdResSKDDecomposition full search threshold: " << _fullSearchThreshold << endl;
	xmlconfig.getNodeValue("heterogeneousSystems", _heterogeneousSystems);
	global_log->info() << "AdResSKDDecomposition for heterogeneous computing systems (old version, not compatible with new "
						  "VecTuner version)?: "
...

   	{
		...
		global_log->info() << "AdResSKDDecomposition uses " << deviationReductionOperation
	...
	global_log->info() << "AdResSKDDecomposition using vectorization tuner: " << (useVecTuner?"yes":"no") << endl;
	if (useVecTuner){
		delete _loadCalc;
	...
	global_log->info() << "AdResSKDDecomposition for clustered heterogeneous systems?: " << (_clusteredHeterogeneouseSystems?"yes":"no") << endl;
	...
	global_log->info() << "AdResSKDDecomposition splits along biggest domain?: " << (_splitBiggest?"yes":"no") << endl;
	xmlconfig.getNodeValue("forceRatio", _forceRatio);
	global_log->info() << "AdResSKDDecomposition forces load/performance ratio?: " << (_forceRatio?"yes":"no") << endl;

	xmlconfig.getNodeValue("rebalanceLimit", _rebalanceLimit);
	if(_rebalanceLimit > 0) {
		global_log->info() << "AdResSKDDecomposition automatic rebalancing: enabled" << endl;
		global_log->info() << "AdResSKDDecomposition rebalance limit: " << _rebalanceLimit << endl;
	}
	else {
		global_log->info() << "AdResSKDDecomposition automatic rebalancing: disabled" << endl;
	}
	xmlconfig.getNodeValue("splitThreshold", _splitThreshold);
	if(!_splitBiggest){
		global_log->info() << "AdResSKDDecomposition threshold for splitting not only the biggest Domain: " << _splitThreshold << endl;
	}
	...
	global_log->info() << "AdResSKDDecomposition using cellsInCutoffRadius: " << _cellsInCutoffRadius << endl;

	// reset path
	xmlconfig.changecurrentnode(oldPath);
}

void AdResSKDDecomposition::prepareNonBlockingStage(bool /*forceRebalancing*/,
...
void AdResSKDDecomposition::finishNonBlockingStage(bool /*forceRebalancing*/,
		ParticleContainer* moleculeContainer, Domain* domain,
		unsigned int stageNumber) {
	const bool removeRecvDuplicates = true;
	if(sendLeavingWithCopies()){
...
bool AdResSKDDecomposition::doRebalancing(bool forceRebalancing, bool needsRebalance, size_t steps, int frequency){
	return forceRebalancing or ((steps % frequency == 0 or steps <= 1) and needsRebalance);
}

bool AdResSKDDecomposition::queryBalanceAndExchangeNonBlocking(bool forceRebalancing, ParticleContainer* /*moleculeContainer*/, Domain* /*domain*/, double etime){
	bool needsRebalance = checkNeedRebalance(etime);
	return not doRebalancing(forceRebalancing, needsRebalance, _steps, _frequency);
}

bool AdResSKDDecomposition::checkNeedRebalance(double lastTraversalTime) const {
	...
	if (_rebalanceLimit > 0) {
		...
		global_log->info() << "AdResSKDDecomposition timerCoeff: " << timerCoeff << endl;
...
void AdResSKDDecomposition::balanceAndExchange(double lastTraversalTime, bool forceRebalancing, ParticleContainer* moleculeContainer, Domain* domain) {
	...
	if (not rebalance) {
		if (sendLeavingWithCopies()) {
			global_log->debug() << "AkDD: Sending Leaving and Halos together." << std::endl;
			DomainDecompMPIBase::exchangeMoleculesMPI(moleculeContainer, domain, LEAVING_AND_HALO_COPIES,
													  true /*doHaloPositionCheck*/, removeRecvDuplicates);
		} else {
			global_log->debug() << "AkDD: Sending Leaving, then Halos." << std::endl;
		...
		global_log->info() << "AdResSKDDecomposition: rebalancing..." << endl;
		if (_steps != 1) {
			DomainDecompMPIBase::exchangeMoleculesMPI(moleculeContainer, domain, LEAVING_ONLY,
													  true /*doHaloPositionCheck*/, removeRecvDuplicates);
		}
...
void AdResSKDDecomposition::initCommunicationPartners(double cutoffRadius, Domain * domain, ParticleContainer* moleculeContainer) {
	_neighbourCommunicationScheme->initCommunicationPartners(cutoffRadius, domain, this, moleculeContainer);
}

void AdResSKDDecomposition::getCellBorderFromIntCoords(double * lC, double * hC, int lo[3], int hi[3]) const {
	for(int d = 0 ; d < 3; ++d) {
...
void AdResSKDDecomposition::getCellIntCoordsFromRegionPeriodic(int* lo, int* hi, const double lC[3], const double hC[3], const Domain* domain) const {
	for (int d = 0; d < 3; ++d) {
...
bool AdResSKDDecomposition::migrateParticles(const KDNode& newRoot, const KDNode& newOwnLeaf, ParticleContainer* moleculeContainer, Domain* domain) {
			...
			for (int iz = low[2]; iz <= high[2]; ++iz) {
				for (int iy = low[1]; iy <= high[1]; ++iy) {
					for (int ix = low[0]; ix <= high[0]; ++ ix) {
						numMols += _numMoleculesPerCell[(iz * _globalCellsPerDim[1] + iy) * _globalCellsPerDim[0] + ix];
						if(_numParticleTypes >= 2){
							numMols += _numMoleculesPerCell[_globalNumCells + (iz * _globalCellsPerDim[1] + iy) * _globalCellsPerDim[0] + ix];
						}
					}
				}
			}
	...
	while (not allDone) {
		...
		if (waitingTime > waitCounter) {
			global_log->warning() << "AdResSKDDecomposition::migrateParticles: Deadlock warning: Rank " << _rank
		...
		if (waitingTime > deadlockTimeOut) {
			global_log->error() << "AdResSKDDecomposition::migrateParticles: Deadlock error: Rank " << _rank
...
void AdResSKDDecomposition::fillTimeVecs(CellProcessor **cellProc){
	if(cellProc == nullptr){
...
void AdResSKDDecomposition::constructNewTree(KDNode *& newRoot, KDNode *& newOwnLeaf, ParticleContainer* moleculeContainer) {
	...
	global_log->info() << "AdResSKDDecomposition: rebalancing finished" << endl;
...
void AdResSKDDecomposition::updateMeanProcessorSpeeds(std::vector<double>& processorSpeeds,
...
double AdResSKDDecomposition::getBoundingBoxMin(int dimension, Domain* domain) {
...
double AdResSKDDecomposition::getBoundingBoxMax(int dimension, Domain* domain) {
...
void AdResSKDDecomposition::completeTreeInfo(KDNode*& root, KDNode*& ownArea) {
...
bool AdResSKDDecomposition::decompose(KDNode* fatherNode, KDNode*& ownArea, MPI_Comm commGroup) {
	return decompose(fatherNode, ownArea, commGroup, std::numeric_limits<double>::max());
}

bool AdResSKDDecomposition::decompose(KDNode* fatherNode, KDNode*& ownArea, MPI_Comm commGroup, const double globalMinimalDeviation) {
...
bool AdResSKDDecomposition::calculateAllPossibleSubdivisions(KDNode* node, std::list<KDNode*>& subdividedNodes, MPI_Comm commGroup) {
	bool domainTooSmall = false;
 ...
 */
void AdResSKDDecomposition::calculateCostsPar(KDNode* area, vector<vector<double> >& costsLeft, vector<vector<double> >& costsRight, MPI_Comm commGroup) {
								...
								*/
								...
								switch(zeroCount1+zeroCount2+zeroCount3){
									...
									case 3: //3 zeroes is the cell itself which was already counted
										break;
									default:
										global_log->error() << "[AdResSKDDecomposition] zeroCounts too large!" << std::endl;
										Simulation::exit(1);
								}
							}
						}
...
unsigned int AdResSKDDecomposition::getGlobalIndex(int divDim, int dim1, int dim2, int index_divDim, int index_dim1, int index_dim2, KDNode* localArea) {
...
int AdResSKDDecomposition::ownMod(int number, int modulo) const {
...
void AdResSKDDecomposition::getOwningProcs(int low[KDDIM], int high[KDDIM], KDNode* decompTree, KDNode* testNode, vector<int>* procIDs, vector<int>* neighbHaloAreas) const {
...
void AdResSKDDecomposition::calcNumParticlesPerCell(ParticleContainer* moleculeContainer) {
	for (int i = 0; i < _globalNumCells * _numParticleTypes; i++) {
        _numMoleculesPerCell[i] = 0;
	...
	#endif
	{
		for(auto molPtr = moleculeContainer->iterator(ParticleIterator::ONLY_INNER_AND_BOUNDARY); molPtr.isValid(); ++molPtr) {
			...
			if(molPtr->componentid() == 0){
				...
				#pragma omp atomic
				#endif
				_numMoleculesPerCell[_globalCellsPerDim[0] * (globalCellIdx[2] * _globalCellsPerDim[1] + globalCellIdx[1]) + globalCellIdx[0]]++;
                #if defined(_OPENMP)
                #pragma omp atomic
                #endif
                _numParticlesPerCell[_globalCellsPerDim[0] * (globalCellIdx[2] * _globalCellsPerDim[1] + globalCellIdx[1]) + globalCellIdx[0]]+=particles;

			} else {
				#if defined(_OPENMP)
				#pragma omp atomic
				#endif
				_numMoleculesPerCell[_globalNumCells + _globalCellsPerDim[0] * (globalCellIdx[2] * _globalCellsPerDim[1] + globalCellIdx[1]) + globalCellIdx[0]]++;
                #if defined(_OPENMP)
                #pragma omp atomic
                #endif
                _numParticlesPerCell[_globalNumCells + _globalCellsPerDim[0] * (globalCellIdx[2] * _globalCellsPerDim[1] + globalCellIdx[1]) + globalCellIdx[0]]+=particles;
			}
		}
	}
...
std::vector<int> AdResSKDDecomposition::getNeighbourRanksFullShell() {
	//global_log->error() << "not implemented \n";
	Simulation::exit(-1);
	return std::vector<int> (0);
}

std::vector<CommunicationPartner> AdResSKDDecomposition::getNeighboursFromHaloRegion(Domain* domain,
...
void AdResSKDDecomposition::collectMoleculesInRegion(ParticleContainer* moleculeContainer, const double startRegion[3], const double endRegion[3], vector<Molecule*>& mols) const {
...
bool AdResSKDDecomposition::heteroDecompose(KDNode* fatherNode, KDNode*& ownArea, MPI_Comm commGroup) {
	bool domainTooSmall = false;
...
int AdResSKDDecomposition::calculatePartitionRank(){
	int procCount;
...
bool AdResSKDDecomposition::calculateHeteroSubdivision(KDNode* node, KDNode*& optimalNode, MPI_Comm commGroup) {
	bool domainTooSmall = false;
	vector<vector<double> > costsLeft(3);
	vector<vector<double> > costsRight(3);
	{
...
void AdResSKDDecomposition::printTree(std::ostream& ostream) {
