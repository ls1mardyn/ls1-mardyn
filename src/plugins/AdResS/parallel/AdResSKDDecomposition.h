#ifndef ADRESSKDDECOMPOSITION2_H_
#define ADRESSKDDECOMPOSITION2_H_
...
/** @brief AdResS aware version of KD tree based domain decomposition for better load balancing.
 *
 ...
 */
class AdResSKDDecomposition: public DomainDecompMPIBase {
 public:
	 ...
	 *                            set it to 2 - 4.
	 */
    AdResSKDDecomposition(double cutoffRadius, int numParticleTypes, int updateFrequency = 100,  int fullSearchThreshold = 2);
...
private:
	...
	//! Number of molecules for each cell (without halo!)
	std::vector<unsigned int> _numMoleculesPerCell;

    //! Number of actual particles i.e. all atoms for each cell
...
#endif /* ADRESSKDDECOMPOSITION2_H_ */
