...
void Sampler::SetMeasureRadius(double r){
    this->measure_radius=r;
}

void Sampler::init(Grid* grid){
...
IdxArray Sampler::GetParticleLocalCellIndices(ParticleIterator it){
...
void Sampler::SampleAtNodes(ParticleContainer* pc){
...
bool Sampler::ParticleInsideMeasuringSpace(std::array<double, 3> nodal_pos, std::array<double, 3> par_pos){
...
std::ostream& Sampler::WriteSample(std::ostream& out, std::vector<double>& smpl){
...
std::ostream& Sampler::WriteSample(std::ostream& out, std::vector<int>& smpl){
    std::string prefix = "//[Sampler]: ";
    out<<prefix+" target value: "<<this->samples.target_number_density<<"\n";
    out<<prefix+"total sampled nodes: "<<smpl.size()<<"\n";
    for(int i=0;i<smpl.size();i++){
...
std::ostream& Sampler::WriteInfo(std::ostream& out){
    std::string prefix ="//[Sampler]: ";
    
}

GridHandler& Sampler::GetGridHandler(){
    return handler;
}

void Sampler::SetSubsetMaterialDensityValues(){
