...
BoundaryHandler::BoundaryHandler()
    : _boundaries{{BoundaryUtils::DimensionType::POSX, BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL},
		  ...
		  {BoundaryUtils::DimensionType::ERROR, BoundaryUtils::BoundaryType::ERROR}} {}

BoundaryUtils::BoundaryType BoundaryHandler::getGlobalWallType(BoundaryUtils::DimensionType dimension) const {
  return _boundaries.at(dimension);
}

BoundaryUtils::BoundaryType BoundaryHandler::getGlobalWallType(std::string dimension) const {
  BoundaryUtils::DimensionType convertedDimension = BoundaryUtils::convertStringToDimension(dimension);
...
BoundaryUtils::BoundaryType BoundaryHandler::getGlobalWallType(int dimension) const {
  return getGlobalWallType(BoundaryUtils::convertLS1DimsToDimensionPos(dimension));
}

void BoundaryHandler::setGlobalWallType(BoundaryUtils::DimensionType dimension, BoundaryUtils::BoundaryType value) {
  if (dimension != BoundaryUtils::DimensionType::ERROR)
    _boundaries[dimension] = value;
}

void BoundaryHandler::setGlobalWallType(std::string dimension, BoundaryUtils::BoundaryType value) {
  BoundaryUtils::DimensionType convertedDimension = BoundaryUtils::convertStringToDimension(dimension);
...
void BoundaryHandler::updateGlobalWallLookupTable() {
  _isGlobalWall[BoundaryUtils::DimensionType::POSX] = BoundaryUtils::isNearRel(_localRegionEnd[0], _globalRegionEnd[0]);
  _isGlobalWall[BoundaryUtils::DimensionType::NEGX] =
      BoundaryUtils::isNearRel(_localRegionStart[0], _globalRegionStart[0]);
  _isGlobalWall[BoundaryUtils::DimensionType::POSY] = BoundaryUtils::isNearRel(_localRegionEnd[1], _globalRegionEnd[1]);
  _isGlobalWall[BoundaryUtils::DimensionType::NEGY] =
      BoundaryUtils::isNearRel(_localRegionStart[1], _globalRegionStart[1]);
  _isGlobalWall[BoundaryUtils::DimensionType::POSZ] = BoundaryUtils::isNearRel(_localRegionEnd[2], _globalRegionEnd[2]);
...
bool BoundaryHandler::hasInvalidBoundary() const {
  return _boundaries.at(BoundaryUtils::DimensionType::POSX) == BoundaryUtils::BoundaryType::ERROR ||
	 _boundaries.at(BoundaryUtils::DimensionType::POSY) == BoundaryUtils::BoundaryType::ERROR ||
	 _boundaries.at(BoundaryUtils::DimensionType::POSZ) == BoundaryUtils::BoundaryType::ERROR ||
	 _boundaries.at(BoundaryUtils::DimensionType::NEGX) == BoundaryUtils::BoundaryType::ERROR ||
	 _boundaries.at(BoundaryUtils::DimensionType::NEGY) == BoundaryUtils::BoundaryType::ERROR ||
	 _boundaries.at(BoundaryUtils::DimensionType::NEGZ) == BoundaryUtils::BoundaryType::ERROR;
}

bool BoundaryHandler::hasNonPeriodicBoundary() const {
  return _boundaries.at(BoundaryUtils::DimensionType::POSX) != BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL ||
	 _boundaries.at(BoundaryUtils::DimensionType::POSY) != BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL ||
	 _boundaries.at(BoundaryUtils::DimensionType::POSZ) != BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL ||
	 _boundaries.at(BoundaryUtils::DimensionType::NEGX) != BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL ||
	 _boundaries.at(BoundaryUtils::DimensionType::NEGY) != BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL ||
	 _boundaries.at(BoundaryUtils::DimensionType::NEGZ) != BoundaryUtils::BoundaryType::PERIODIC_OR_LOCAL;
}

bool BoundaryHandler::isGlobalWall(BoundaryUtils::DimensionType dimension) const { return _isGlobalWall.at(dimension); }
...
void BoundaryHandler::processGlobalWallLeavingParticles(ParticleContainer *moleculeContainer, double timestepLength) {
  ...
  for (auto const [currentDim, currentWallIsGlobalWall] : _isGlobalWall) {
    ...
    case BoundaryUtils::BoundaryType::REFLECTING: {
	  ...
	  BoundaryUtils::getInnerBuffer(_localRegionStart, _localRegionEnd, currentWall.first, cutoff);
      // convert the regions into c-style arrays, so that they can be passed to
      // the region iterator
      const double cStyleRegionBegin[] = {curWallRegionBegin[0], curWallRegionBegin[1], curWallRegionBegin[2]};
      const double cStyleRegionEnd[] = {curWallRegionEnd[0], curWallRegionEnd[1], curWallRegionEnd[2]};

      // grab an iterator from the converted coords
      auto particlesInRegion = moleculeContainer->regionIterator(cStyleRegionBegin, cStyleRegionEnd,
	...
	const int currentDim = BoundaryUtils::convertDimensionToLS1Dims(currentWall.first);
	...
	if (BoundaryUtils::isMoleculeLeaving(curMolecule, curWallRegionBegin, curWallRegionEnd, currentWall.first,
					     timestepLength, nextStepVelAdjustment)) {
	  if (getGlobalWallType(currentWall.first) == BoundaryUtils::BoundaryType::REFLECTING) {
...
void BoundaryHandler::removeNonPeriodicHalos(ParticleContainer *moleculeContainer) {
  ...
  double buffers[] = {moleculeContainer->get_halo_L(0), moleculeContainer->get_halo_L(1),
  ...
  for (auto const [currentDim, currentWallIsGlobalWall] : _isGlobalWall) {
    ...
    case BoundaryUtils::BoundaryType::REFLECTING: {
	  ...
	  BoundaryUtils::getOuterBuffer(_localRegionStart, _localRegionEnd, currentWall.first, buffers);
      // convert the regions into c-style arrays, so that they can be passed to
      // the region iterator
      const double cStyleRegionBegin[] = {curWallRegionBegin[0], curWallRegionBegin[1], curWallRegionBegin[2]};
      const double cStyleRegionEnd[] = {curWallRegionEnd[0], curWallRegionEnd[1], curWallRegionEnd[2]};
      ...
      // grab an iterator from the converted coords
      auto particlesInRegion =
	  moleculeContainer->regionIterator(cStyleRegionBegin, cStyleRegionEnd, ParticleIterator::ALL_CELLS);
      for (auto it = particlesInRegion; it.isValid(); ++it) {
