...
bool KDDecomposition::recDecompPar(KDNode* fatherNode, KDNode*& ownArea, MPI_Comm commGroup){
  ...
  for(int dim=0; dim<3; dim++){
    ...
    for(int i=1; i<fatherNode->_highCorner[dim]-fatherNode->_lowCorner[dim]-1; i++){
      ...
      if(numProcsLeftTest <= maxProcsLeft && numProcsRightTest <= maxProcsRight) ok = true;
      if(not(ok)){
        domainTooSmall = true;
        procShiftAllowed = false;
        int shiftDir;
        if(maxProcsLeft < numProcsLeftTest) shiftDir = 1; // move procs to right
        else shiftDir = -1; // move procs to left
        while (not(ok)){
          numProcsLeftTest -= shiftDir;
          numProcsRightTest += shiftDir;
          if(maxProcsLeft >= numProcsLeftTest && maxProcsRight >= numProcsRightTest) ok = true;
        }
      }
      ...
      if(procShiftAllowed){
        while(maxProcCostNew <= maxProcCostOld){
          ...
          if(numProcsLeftTest  >= fatherNode->_numProcs || numProcsLeftTest  <= 0 ||
              numProcsRightTest >= fatherNode->_numProcs || numProcsRightTest <= 0){
            break;
          }
          else{
...
void KDDecomposition::completeTreeInfo(KDNode*& root, KDNode*& ownArea){
  ...
  for(int nodeID = 0; nodeID < numElementsToRecv; nodeID++){
    ...
    bool coversAll[3];
    for(int i=0; i<3; i++) coversAll[i] = false;
    if(data[7]>=4){
      coversAll[0] = true;
      data[7] -= 4;
    }
...
void KDDecomposition::calculateCostsPar(KDNode* area, vector<vector<double> >& costsLeft, vector<vector<double> >& costsRight, bool calcDivisionCosts, MPI_Comm commGroup){
  ...
  for(int dim=0; dim<3; dim++){
    ...
    if(loopstart > 0) recvCostValue = true;
    if(loopend < area->_highCorner[dim]-area->_lowCorner[dim]) sendCostValue = true;


    for(int i_dim=loopstart; i_dim<=loopend; i_dim++){

      if(i_dim==0) cellCosts[dim][i_dim] = 0;
      else cellCosts[dim][i_dim] = cellCosts[dim][i_dim-1];
      ...
      for(int i_dim1 = 0; i_dim1 <= area->_highCorner[dim1]-area->_lowCorner[dim1]; i_dim1++){
        for(int i_dim2 = 0; i_dim2 <= area->_highCorner[dim2]-area->_lowCorner[dim2]; i_dim2++){
          ...
          for(int neighInd_divDim = i_dim-1; neighInd_divDim<=i_dim+1; neighInd_divDim++){
            ...
            if(nI_dim+area->_lowCorner[dim]<0) nI_dim = _globalCellsPerDim[dim]-1;
            if(nI_dim+area->_lowCorner[dim]>=_globalCellsPerDim[dim]) nI_dim = 0;
            for(int neighInd_dim1 = i_dim1-1; neighInd_dim1<=i_dim1+1; neighInd_dim1++){
              ...
              if(nI_dim1+area->_lowCorner[dim1]<0) nI_dim1 = _globalCellsPerDim[dim1]-1;
              if(nI_dim1+area->_lowCorner[dim1]>=_globalCellsPerDim[dim1]) nI_dim1 = 0;

              for(int neighInd_dim2 = i_dim2-1; neighInd_dim2<=i_dim2+1; neighInd_dim2++){
                // adjust index in case of periodic boundary
                int nI_dim2 = neighInd_dim2;
                if(nI_dim2+area->_lowCorner[dim2]<0) nI_dim2 = _globalCellsPerDim[dim2]-1;
                if(nI_dim2+area->_lowCorner[dim2]>=_globalCellsPerDim[dim2]) nI_dim2 = 0;
          ...
          if (neighInd_divDim+area->_lowCorner[dim] == _globalCellsPerDim[dim]) neighInd_divDim = 0;
...
void KDDecomposition::getOwningProcs(int low[KDDIM], int high[KDDIM], KDNode* decompTree, KDNode* testNode, vector<int>* procIDs, vector<int>* neighbHaloAreas){
  ...
  for(int dim=0; dim<KDDIM; dim++){
    ...
    // @todo Do this nicer
    if(high[dim]-low[dim]>=decompTree->_highCorner[dim]-decompTree->_lowCorner[dim]) coversWholeDomain[dim] = true;
    else coversWholeDomain[dim] = false;
    ...
    if(coversWholeDomain[dim]) low[dim] = high[dim] + 1;
  }
