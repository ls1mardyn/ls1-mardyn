...
void Ensemble::readXML(XMLfileUnits& xmlconfig) {
	...
	global_log->info() << "Number of components: " << numComponents << std::endl;
	if (numComponents == 0) {
	...
	for(componentIter = query.begin(); componentIter; componentIter++) {
		...
		global_log->debug() << _components[cid - 1].getName() << " --> " << cid - 1 << std::endl;
	}
	...
	global_log->info() << "Found " << numMixingrules << " mixing rules." << std::endl;
	...
	for(mixingruletIter = query.begin(); mixingruletIter; mixingruletIter++) {
		...
		global_log->info() << "Mixing rule type: " << mixingruletype << std::endl;
		if("LB" == mixingruletype) {
			mixingrule = new LorentzBerthelotMixingRule();

		} else {
			global_log->error() << "Unknown mixing rule " << mixingruletype << std::endl;
			Simulation::exit(1);
		}
