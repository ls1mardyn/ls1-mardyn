...
ReturnType SnapshotCommand::execute ()
{
  ...
  //std::cout << "----------------->SnapshotCommand::execute for rank " << myRank << " of " << mySize << std::endl;
  //return EXECUTED;
#else
  ...
  //std::cout << "snapshotCommand (stream) is executed" << std::endl;
...
//std::cout << "in getSnapshot we have "<< local_mol << " particles from " << anzahl_mol  << std::endl;
  ...
  if (myRank == 0)
  {
    ...
    BaseSimSteering::comm->sendStream (this->getConnection()->getDataConnection(), outStream);
...
void SnapshotCommand::setParameters (std::list <std::string> params)
{
  ...
  if (listSize > 4)
  {
    std::cout << "got Parameters: " << stepInterval << " " << sendForces << " " << sendVelocity << " " << sendV2  << " "
    << sendV2Max <<  std::endl;
  }
}
