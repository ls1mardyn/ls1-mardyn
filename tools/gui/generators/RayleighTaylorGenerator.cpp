...
RayleighTaylorGenerator::RayleighTaylorGenerator() :
	...
	_components[0].addLJcenter(0, 0, 0, 1.0, 1.0, 1.0, 0.0, false);
	_components[1].addLJcenter(0, 0, 0, 1.0, 1.0, 1.0, 0.0, false);
}
...
//	std::cout << "from RayleighTaylor::readPhaseSpace " << std::flush;
	...
	for(int idx = 0; idx < _N; idx++){
		...
		double x = _L1 * ((double) rand() / (double) RAND_MAX);
		double y = _L2 * ((double) rand() / (double) RAND_MAX);
		double z = _L3 * ((double) rand() / (double) RAND_MAX);
...
vector<ParameterCollection*> RayleighTaylorGenerator::getParameters() {
	...
	tab->addParameter(
			new ParameterWithDoubleValue("SimulationBoxSize(X)", "SimulationBoxSize(X)",
					"SimulationBoxSize(X)", Parameter::LINE_EDIT, false, _L1));

	tab->addParameter(
			new ParameterWithDoubleValue("SimulationBoxSize(y)", "SimulationBoxSize(y)",
					"SimulationBoxSize(y)", Parameter::LINE_EDIT, false, _L2));

	tab->addParameter(
			new ParameterWithDoubleValue("SimulationBoxSize(Z)", "SimulationBoxSize(Z)",
					"SimulationBoxSize(Z)", Parameter::LINE_EDIT, false, _L3));
	...
	tab->addParameter(
			new ParameterWithDoubleValue("Temperature[K]", "Temperature[K]",
					"Temperature[K]", Parameter::LINE_EDIT, false, _T));
...
void RayleighTaylorGenerator::setParameter(Parameter* p) {
	...
	if (id == "SimulationBoxSize(X)") {
		_L1 = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: SimulationBoxSize(X): " << _L1
				<< endl;

	} else if (id == "SimulationBoxSize(y)") {
		_L2 = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: SimulationBoxSize(y): " << _L2
				<< endl;

	} else if (id == "SimulationBoxSize(Z)") {
		_L3 = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: SimulationBoxSize(Z): " << _L3
				<< endl;

	} else if (id == "epsilon_A") {
		epsilon_A = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: epsilon_A: " << epsilon_A
				<< endl;

	} else if (id == "epsilon_B") {
		epsilon_B = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: epsilon_B: " << epsilon_B
				<< endl;

	} else if (id == "sigma_A") {
		sigma_A = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: sigma_A: " << sigma_A
				<< endl;

	} else if (id == "sigma_B") {
		sigma_B = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: sigma_B: " << sigma_B
				<< endl;

	} else if (id == "m_A") {
		sigma_B = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: m_A: " << m_A
				<< endl;

	} else if (id == "m_B") {
		sigma_B = static_cast<ParameterWithDoubleValue*> (p)->getValue();
		cout << "OneCenterLJRayleighTaylor: m_B: " << m_B
				<< endl;
		...
		_T = static_cast<ParameterWithDoubleValue*> (p)->getValue();
