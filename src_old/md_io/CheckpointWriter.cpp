...
md_io::CheckpointWriter::CheckpointWriter(unsigned long writeFrequency, string filename, unsigned long numberOfTimesteps, bool incremental) {
   _filename = filename;
...
md_io::CheckpointWriter::~CheckpointWriter(){}

void md_io::CheckpointWriter::initOutput(datastructures::ParticleContainer<Molecule>* particleContainer,
                         parallel::DomainDecompBase* domainDecomp, Domain* domain) {
}

void md_io::CheckpointWriter::doOutput(datastructures::ParticleContainer<Molecule>* particleContainer,
   ...
   if(simstep%_writeFrequency == 0) {
	 ...
	 filenamestream << ".inp";
      ...
      checkpointfilestream << "Temperature\t" 
	      << domain->getGlobalTemperature() << endl;
      checkpointfilestream << "Length\t" 
      ...
      checkpointfilestream << "NumberOfComponents\t" 
	      << dcomponents.size() << endl;
      for(vector<Component>::const_iterator pos=dcomponents.begin();
		      pos!=dcomponents.end();++pos){
         pos->write(checkpointfilestream);
      }
      unsigned int numperline=dcomponents.size();
      unsigned int iout=0;
      vector<double> dmixcoeff = domain->getmixcoeff();
      for(vector<double>::const_iterator pos=dmixcoeff.begin();
		      pos!=dmixcoeff.end();++pos){
...
void md_io::CheckpointWriter::finishOutput(datastructures::ParticleContainer<Molecule>* particleContainer,
                         parallel::DomainDecompBase* domainDecomp, Domain* domain){
}
