...
FFTAcceleration_matrices_optFFT::FFTAcceleration_matrices_optFFT(int order) {
	...
	_optFFT_API = optFFT_API_Factory::getOptFFT_API(order,
			(_fft_nx == 2 * _fft_ny));
}

void FFTAcceleration_matrices_optFFT::FFT_initialize_Source(
...
void FFTAcceleration_matrices_optFFT::FFT_initialize_TransferFunction(
...
void FFTAcceleration_matrices_optFFT::FFT_finalize_Target(
		FFTAccelerableExpansion & Expansion, double radius) {
	FFTDataContainer_matrices* FFTData = getFFTData(Expansion);
	FFT_precision** & Re = FFTData->Re;
	FFT_precision** & Im = FFTData->Im;
